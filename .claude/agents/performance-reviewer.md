...
- [ ] Iterating a Wagtail `StreamValue` (`for bound in obj.body`) that contains a `ChooserBlock` (image/document/page) is a per-object N+1 — Wagtail's `_prefetch_blocks` runs `bulk_to_python` → `Image.objects.in_bulk()` per row, and it fires from the iteration itself (not from touching `.value`), so `prefetch_renditions` on the post queryset can't reach the ids (they're inside the JSON). Fix: serialize from `stream_value.raw_data`, collect ids, batch-fetch once into an `{id: obj}` map (todo 231 / `docs/LEARNINGS.md` 2026-06-25)
