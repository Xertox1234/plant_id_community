...
description: Reviews changed Flutter Dart files for Riverpod patterns, memory leaks, Material 3 compliance, and null safety. Dispatched for non-Firebase plant_community_mobile/**/*.dart changes.
model: sonnet
color: blue
...
### Brand / native-shell additions (2026-09-14, #767/#768)

- [ ] **A rename of `AppBrand.name` also touches the native manifests** — `CFBundleDisplayName` (`ios/Runner/Info.plist`) and `android:label` (`AndroidManifest.xml`) are unreachable from any Dart constant; a Dart-only rename leaves the home-screen caption stale, as iOS build 10 shipped it
- [ ] **App icons are full-bleed and alpha-free on both platforms** — both launchers mask, so a pre-rounded tile is rounded twice; Apple rejects an alpha channel outright. Icons must be generated by `scripts/design/render_brand_assets.mjs`, never hand-exported
- [ ] **Launch background matches the first painted frame** — the Flutter defaults are white on both platforms; against a dark first frame that flashes white on every cold start
