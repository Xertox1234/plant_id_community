...
- **Auth is cookie-based** — the session cookie is set by Django on login; do not send an `Authorization: Bearer` header. All services authenticate via cookie only.
