...
| `FIREBASE_CREDENTIALS_PATH` | `backend/.env` | Same JSON, read by `apps/core/firebase_config.py` — gates FCM push; unset = push disabled |
| `FIREBASE_CREDENTIALS_B64` | Railway | base64 of the service-account JSON. `bin/start.sh` decodes it to `/tmp/firebase-service-account.json` (0600) at boot and exports `FIREBASE_CREDENTIALS_PATH` for both children, because the Admin SDK takes a **path** and the repo is public so the JSON cannot be committed. An explicitly-set `FIREBASE_CREDENTIALS_PATH` wins (the volume route). Both unset = **FCM push silently disabled on iOS and Android** — every send returns early at `logger.debug` (todo 286) |
...
| `FORUM_RAG_ENABLED` | `backend/.env` | `True` enables the premium RAG plant-care answers (`POST /forum/care/ask/`, M13 / todo 289); default `False` (endpoint 503s `code: disabled`). A **two-flag gate**: also needs `FORUM_VECTOR_SEARCH_ENABLED` + built `SimilarTopics` **and** `BlogChunks` indexes (`manage.py rebuild_indexes SimilarTopics BlogChunks`). Ships dark by design — enable only per todo 330's procedure (corpus, review-queue owner, floor tuning). Bounded by `IsPremiumUser`, the per-user `care_ask` throttle (10/h) and `RAG_BUDGET_LIMIT`; the wrong-answer report endpoint is deliberately not flag-gated |
| `FORUM_EMBEDS_ENABLED` | `backend/.env` | `True` (default) enables the forum `embed` body block (YouTube/Vimeo links unfurl into sandboxed players, todo 344); `False` refuses the block on write and serves legacy embeds as plain links. The provider allowlist is `WAGTAILEMBEDS_FINDERS` in settings (youtube + vimeo only) |
| `FORUM_LINK_PREVIEWS_ENABLED` | `backend/.env` | `True` (default) turns a link posted on its own into a stored **link card** (todo 428): the host fetcher (`apps.forum_host.link_preview.link_preview_snapshot`, SSRF-pinned) snapshots the page's title, description and site once at write time and caches OUR re-encoded WebP of its image under `forum/link-previews/`; reads never fetch. `manage.py prune_link_preview_images` (`--dry-run`) deletes cached images no `Post.body` or post revision refers to, never one younger than 24 h; it runs from the nightly `forum-prune-cron`. `False` is the kill switch: links stay auto-linked paragraphs, stored cards still render and survive edits. Always off under test runs (`_IS_TEST_RUN`); a test that wants cards overrides `WAGTAILFORUM_LINK_PREVIEW_FETCHER` |
| `USE_R2` | `backend/.env` | `True` routes media (uploads + Wagtail renditions) through Cloudflare R2 instead of local `MEDIA_ROOT` (todo 305); default `False` = dev/local-disk behavior unchanged. Needs `R2_BUCKET_NAME`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_ENDPOINT_URL`, `R2_CUSTOM_DOMAIN` — `validate_environment()` fails fast if any is missing while the flag is on in production. Must be set identically on the `forum-prune-cron` Railway service, which imports the same settings |
| `API_PUBLIC_URL` | `backend/.env` | This API's public origin (`https://api.houseplant-md.com` in prod). Set, reply and digest emails carry an RFC 8058 one-click `List-Unsubscribe` URL on it plus `List-Unsubscribe-Post`; unset (default), the header points at the web `/unsubscribe` page and one-click is off (todo 416). Not the web origin: that is `SITE_URL` |
