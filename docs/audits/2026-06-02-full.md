...
| H4 | Blog comment create/delete/approve never invalidates cached blog responses — no `BlogComment` signal exists. Cached detail/list embed `comment_count`; stale up to 24h TTL. | performance | performance | `apps/blog/signals.py` | — | verified | added `BlogComment` post_save/delete receiver → invalidate post+lists+popular; 178 blog tests pass |
...
| M8 | `BlogCategory` rename/update leaves stale cached blog responses — category-invalidation receiver is commented out. | performance | performance | `apps/blog/signals.py:135-147` | — | verified | activated `BlogCategory` post_save/delete receiver (category+lists invalidation); 178 blog tests pass |
...
| — | **Noted, not fixed:** `statistics` aggregates per-unit on keys `"lbs"/"bunches"` but the model's `HARVEST_UNITS` are `"lb"/"bunch"` — so lb/bunch harvests never appear in `total_quantity_by_unit`. Pre-existing data bug; out of M10's behavior-preserving scope. Candidate for a future todo. | `apps/garden_calendar/api/views.py` `statistics` | open (informational) |
...
- [ ] #H1 Celery autoretry inert (service swallows) → todo 208
- [ ] #M12 rate-limit retry exhaustion stuck pending → todo 208
- [ ] #H2 wagtail-ai get_ai_text broken → todo 204
- [ ] #H3 AI rate-limit wired to no live path → todo 204
- [ ] #L15 dead v2 AI module → todo 204
- [ ] #M1 CareLog activity_type_display dropped → todo 205
- [ ] #M3 CreateTopic non-atomic → todo 205
- [ ] #M4 CreatePost non-atomic → todo 205
- [ ] #M6 firebase_token_exchange no rate-limit → todo 205
- [ ] #M7 forum_stats uncached → todo 205
- [ ] #M13 celery acks_late missing → todo 205
