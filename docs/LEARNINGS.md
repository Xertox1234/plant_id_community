...
## 2026-09-06 — A clean SAST backlog is not a clean log: CodeQL flagged 16 logging sites and missed all four that leaked credentials (todo 354)

Triaging the 43-alert CodeQL backlog turned up **four live credential leaks that
were not among the 43**, while all 16 alerts CodeQL *did* raise under
`py/clear-text-logging-sensitive-data` were false positives or low-grade PII:
nine garden coordinates, two an API key's *length*, one an int, one a species
name, three an int/bool/file-path in dev scripts.

The four real ones, all verified by executing the path, not by reading it:

1. `settings.py` logged the whole `REDIS_URL` on every process start. A Railway
   Redis URL carries the password in its userinfo component, and
   `validate_environment()` is unconditional — so gunicorn and the co-located
   Celery worker each wrote it into the log stream on every boot.
2. `OPENWEATHER_API_KEY` (query parameter `appid`), via
   `logger.error(f"...: {e}")` in both weather services.
3. `TREFLE_API_KEY` — `trefle_service.py` puts the key in `session.params`, so
   it rides **every** request, and the handler logged the URL and the exception.
4. `PLANTNET_API_KEY` (query parameter `api-key`), at two handlers.

Leaks 2–4 are all one mechanism. `requests` builds its exception message from
the **prepared** URL:

```text
str(HTTPError) == "401 Client Error: Unauthorized for url: https://.../search?token=<THE KEY>&q=rose"
```

So `logger.error(f"...: {e}")` writes every query-string credential to the log,
and so does `logger.exception(...)` — the traceback formats the same message.
Nothing at the log site mentions a key; the taint enters through a third-party
`__str__`. That is precisely why CodeQL missed all three, and why a human
reading the line misses them too.

**The rule that follows: audit log sinks by what the value *contains*, not what
it is named.** Two value classes carry credentials while looking inert:

- Any URL that may have a userinfo component or query string — Redis, database,
  broker and signed asset URLs. Log the parsed `hostname`/`port`/`path`.
- Any exception from an HTTP client. Log `type(e).__name__` and
  `e.response.status_code`. Header-authenticated clients (Plant.id, plant.health,
  Unsplash, Pexels) are safe by construction — the credential never enters the
  URL — which is a reason to prefer header auth, not a reason to skip the audit.

Corollary for triage work generally: **finishing a scanner's backlog is not the
same as auditing what the scanner was pointed at.** The 43 alerts were ~95%
noise; all of this todo's security value came from reading the flagged
*neighbourhoods* and noticing what was not flagged. The last two leaks were
found only after the fix shipped, by listing every file the new JIT trigger
would fire on. Budget triage time for reading around a cluster, not just for
dispositioning it.

Closed by PRs #670 and #671; rules in `docs/rules/security.md` with triggers
`requests-exception-interpolated` and `connection-url-in-log`.

## 2026-09-06 — A lockfile regenerated by an older local npm silently drops `libc`; take the bot's lockfile (todo 355 slice 3)

PR #669 and Dependabot's #666/#667 bumped root `wrangler` to the same 4.129.0
and resolved an **identical** tree (`undici` 7.29.0, `sharp` 0.35.2, `workerd`
1.20260903.1). The lockfiles still differed, in two ways only a blob-hash
comparison surfaced:

- My local `npm install` **deleted all 16 `libc` entries** that `main` already
  carried. They are the glibc/musl discriminators on `@img/sharp-linux-*` vs
  `@img/sharp-linuxmusl-*`; without them npm cannot pick the musl binary on an
  Alpine host, and the failure is a runtime "cannot load shared library", not an
  install error.
- It rewrote the lock's `"name"` from `repo` to `plant_id_community`. The root
  `package.json` declares no `name`, so npm falls back to the **checkout
  directory name** — `repo` in the bot's and CI's sandbox, the real directory
  locally. Pure churn the next bot run flips back.

Neither shows up in `npm audit`, in CI, or in a normal diff read: 49 deleted
lines that look like whitespace inside a 40k-line file.

**Rule: when a bot PR and a hand-made PR deliver the same versions, keep the
bot's lockfile** — it is written by the npm that CI and the bot both run.
Compare with `git rev-parse origin/<branch>:package-lock.json`, not by eye, and
...
## 2026-09-09 — A LEARNINGS entry did not stop me repeating the bug it describes (PR #718, GitHub cleanup)

Closing Dependabot #125 (`sharp < 0.35.4`) meant regenerating the root
lockfile. The commit that did it **reproduced, verbatim, the regression this
file already records** under *"2026-09-06 — A lockfile regenerated by an older
local npm silently drops `libc`"*: local npm 11.6.0 stripped all 16 `libc`
fields from the `@img/sharp-linux-*` / `@img/sharp-linuxmusl-*` /
`@img/sharp-libvips-*` entries.

**All 17 CI checks passed on the broken lockfile**, including the Cloudflare
Workers build — exactly as the 2026-09-06 entry predicted ("invisible to
`npm audit`, in CI, or in a normal diff read"). It was caught only by reading
LEARNINGS while preparing an unrelated codify step.

**Root cause of the repeat: the lesson was unreachable at write time.**
`docs/rules/security.md` is 26,231 bytes and `inject-patterns.sh` caps the
injected context at ~8,800, of which `_discipline.md` always consumes 4,426.
Measured by running the hook on a real `package.json` edit rather than reasoning
about it: **8,901 bytes emitted of 30,723 assembled, truncated mid-sentence at
`security.md:59`.** So the npm-audit rules that already live at
`security.md:78-93` have *never once been injected*. Appending another bullet
there would have been inert too.

The delivery channel that works is `docs/rules/triggers.json`: the hook writes
matched triggers at step (3), **before** the domain-rule dump at step (4), so a
trigger survives the truncation that eats the rules. Verified the new
`npm-lockfile-regen-drops-libc` trigger lands at byte offset **4,453** — inside
the cap — on the literal fragment this PR added. Sibling of the todo-369
finding; the general rule is now: *if the target rule file is over the cap,
prose there is decoration — ship the lesson as a trigger.*

**The verification gap that let it through.** The original check compared the
lockfiles at the JSON level and reported "99 packages, 0 added, 0 removed, 27
version changes" — then dismissed "27 packages with non-version field changes"
as `resolved`/`integrity` churn. `libc` was inside that bucket. **Comparing
version fields is not comparing lockfiles.** The check that actually
discriminates is *did any package lose a field*:

```python
lost = [(k, sorted(set(old[k]) - set(new[k])))
        for k in set(old) & set(new) if set(old[k]) - set(new[k])]
```

It printed the 16 dropped `libc` entries immediately. `grep -c '"libc"'` is the
cheap version of the same question.

**When Dependabot says a patch exists but cannot reach it, the fix is
`overrides`.** #125's job failed on *every* run — *"a patched version exists for
sharp, but the available update path still resolves it"* — because every
miniflare alpha through the newest pins sharp at exactly `0.35.2` while the
libheif patch is `0.35.4`. Upgrading wrangler does not move it, and
`npm audit fix --force` "solves" it by downgrading wrangler 4.129.0 → 4.15.2, a
breaking change for a dev-only CVSS 0.0 advisory. An `overrides` entry is the
proportionate answer — but it is a standing instruction with no expiry, the
exact shape `.github/security-suppressions.yml` exists to prevent, so it needs a
removal trigger (todo 381) and an inline note. `^0.35.4` resolves `<0.36.0`: if
miniflare later moves to 0.36.x the override silently caps the tree below what
miniflare declares, and npm applies an override with **no warning**.

**The root `package.json` declared no `name`**, so npm derived the lockfile name
from the checkout directory — which is why it flip-flopped between `repo` (the
bot's and CI's sandbox) and the local directory name on alternating runs.
Declaring `"name"` makes it deterministic everywhere and ends that churn. The
Cloudflare Worker name comes from `wrangler.jsonc`, so nothing downstream reads
it.

### GitHub housekeeping, same session

- **`GET /actions/cache/usage` lags badly and will tell you a prune failed.**
  After 156 successful DELETEs it still reported *168 caches / 9.97 GiB*; the
  list endpoint reported the truth, *13 / 2.36 GiB*. Verify a prune against
  `GET /actions/caches` (`total_count` + a re-sum of `size_in_bytes`), never the
  usage metric. The repo had been sitting at **9.99 GiB of the 10 GB cap**, so
  every run was evicting a live cache.
- **Every `refs/pull/*` cache is dead the moment its PR closes.** Cache scoping
  is one-directional — a PR run may read the base branch's caches, but a
  `refs/heads/main` run can never read a PR-scoped one. With 0 open PRs, all 55
  were unreachable by definition: the safest possible prune target.
- **Do not prune `setup-python` caches by "newest N by last access".** This repo
  runs **two Python versions concurrently** (3.13.9 for the heavy backend venv,
  ~0.19 GiB; 3.13.15 for a lighter job, ~0.02 GiB) and both were hit within the
  same minute. A flat newest-4 rule kept three 3.13.9 caches and would have
  deleted the live 3.13.15 one. Group by python version first, then keep the
  newest per version; `codeql-overlay-base` is keyed per (language, commit), so
  group by language.
