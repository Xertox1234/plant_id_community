...
if not settings.configured:
    ...
    if not secret_key or secret_key.startswith("django-insecure"):
        ...
        print(
            "WARNING: Using generated SECRET_KEY. Set SECRET_KEY in .env for persistence."
        )
    ...
    if not plant_id_key:
        raise ValueError(
            "PLANT_ID_API_KEY not found in environment variables. Please set in .env file."
        )
    if not plantnet_key:
        raise ValueError(
            "PLANTNET_API_KEY not found in environment variables. Please set in .env file."
        )
