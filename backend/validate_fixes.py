...
def validate_exception_class():
    ...
    try:
        ...
        expected_str = (
            "Trefle: Rate limit exceeded (121/120 calls/hour) (retry after 60s)"
        )
        actual_str = str(exc)
        ...
        if expected_str == actual_str:
            ...
            return (
                False,
                f"Exception string mismatch. Expected: {expected_str}, Got: {actual_str}",
            )
...
def validate_rate_limiter_logic():
    ...
    try:
        ...
        if len(current_calls) >= max_calls_per_hour:
            ...
            return (
                True,
                f"Rate limit logic works: {len(current_calls)} calls, retry after {retry_after}s",
            )
        else:
...
def validate_monitoring_logic():
    ...
    try:
        ...
        if final_calls == 2:
            ...
            return (
                False,
                f"Monitoring logic failed: expected 2 calls, got {final_calls}",
            )
...
def main():
    ...
    all_passed = (
        syntax_tests_passed == syntax_tests_total
        and logic_tests_passed == logic_tests_total
    )
