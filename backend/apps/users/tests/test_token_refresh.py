...
class TokenRefreshTestCase(TestCase):
    ...
    def test_old_refresh_token_blacklisted(self):
        ...
        # Refresh token
        csrf_token = login_response.cookies.get("csrftoken").value
        self.client.post("/api/v1/auth/token/refresh/", HTTP_X_CSRFTOKEN=csrf_token)
    ...
    def test_token_refresh_rate_limiting(self):
        ...
        for i in range(11):
            ...
            if i < 10:
                # First 10 are within the 10/h limit and refresh successfully.
                self.assertEqual(response.status_code, status.HTTP_200_OK)
            else:
                # 11th exceeds the limit and is rate limited.
                self.assertEqual(
...
class TokenLifetimeTestCase(TestCase):
    ...
    def test_security_window_reduced(self):
        ...
        access_lifetime_seconds = settings.SIMPLE_JWT[
        ...
        print(
            f"  Improvement over 24h tokens: {improvement_factor:.0f}x smaller window"
        )
        print(
            f"  Phase 2 target: {phase2_target/60:.0f} minutes ({old_lifetime/phase2_target:.0f}x improvement)"
        )
