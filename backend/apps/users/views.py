...
def care_reminders(request: Request) -> Response:
    ...
    if request.method == "GET":
        ...
        reminders = (
            CareReminder.objects.filter(user=request.user, is_active=True)
        ...
        return Response({"reminders": reminder_data, "total_count": len(reminder_data)})
        ...
        try:
            ...
            care_instructions = SavedCareInstructions.objects.get(
                id=care_instructions_id, user=request.user
            )
...
def care_reminder_detail(request: Request, reminder_uuid: str) -> Response:
    ...
    try:
        reminder = CareReminder.objects.select_related("saved_care_instructions").get(
            uuid=reminder_uuid, user=request.user
        )
    except CareReminder.DoesNotExist:
        return Response(
            {"error": "Care reminder not found"}, status=status.HTTP_404_NOT_FOUND
        )
...
def care_reminder_action(request: Request, reminder_uuid: str) -> Response:
    ...
    try:
        reminder = CareReminder.objects.get(uuid=reminder_uuid, user=request.user)
    except CareReminder.DoesNotExist:
        return Response(
            {"error": "Care reminder not found"}, status=status.HTTP_404_NOT_FOUND
        )
    ...
    if action == "complete":
        ...
        try:
            ...
            return Response(
                {"error": "snooze_hours must be an integer"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        if not (1 <= snooze_hours <= 8760):
            return Response(
                {"error": "snooze_hours must be between 1 and 8760 (1 hour to 1 year)"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
def care_reminder_stats(request: Request) -> Response:
    ...
    # Basic counts
    active_reminders = CareReminder.objects.filter(
    ...
    thirty_days_ago = timezone.now() - timedelta(days=30)
    recent_logs = (
        CareReminderLog.objects.filter(
            reminder__user=request.user, created_at__gte=thirty_days_ago
        )
        .values("action")
    ...
    upcoming = (
        CareReminder.objects.filter(
            user=request.user, is_active=True, next_reminder_date__gte=timezone.now()
        )
        .select_related("saved_care_instructions")
    ...
    return Response(
        {
            "summary": {
                ...
                "completion_rate": (
                    (completion_stats["total_completed"] or 0)
                    / max(completion_stats["total_sent"] or 1, 1)
                    * 100
                ),
...
def export_care_reminders_calendar(request: Request) -> HttpResponse:
    ...
    for reminder in reminders:
        ...
        while current_date <= end_date:
            ...
            plant_name = _sanitize_ics_field(
                reminder.saved_care_instructions.display_name
            )
            reminder_type = _sanitize_ics_field(reminder.get_reminder_type_display())
            description = _sanitize_ics_field(
                reminder.description
                or f"Time to {reminder_type.lower()} your {plant_name}"
            )
                ...
                calendar_events.append(
                    {
                        ...
                        "can_complete": current_date
                        <= timezone.now()
                        + timedelta(hours=2),  # Allow completion 2 hours early
