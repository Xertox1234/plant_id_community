...
class PlantSpeciesAPIViewSet(BaseAPIViewSet):
    ...
    def families(self, request):
        ...
        families = (
            PlantSpecies.objects.values_list("family", flat=True)
            .distinct()
            .exclude(family="")
            .order_by("family")
        )
        ...
        return Response(
            [
                {
                    "name": family,
                    "count": PlantSpecies.objects.filter(family=family).count(),
                }
    ...
    def plant_types(self, request):
        ...
        plant_types = (
            PlantSpecies.objects.exclude(plant_type="")
...
class PlantCategoryAPIViewSet(BaseAPIViewSet):
    ...
    known_query_parameters = BaseAPIViewSet.known_query_parameters.union(["featured"])

    def get_queryset(self):
        """Enhanced queryset with category filtering."""
        # `_plant_count` annotation feeds PlantCategorySerializer.get_plant_count
        # without a COUNT query per category (replaces the prefetch that the
        # count previously relied on).
        queryset = PlantCategory.objects.annotate(
...
class PlantCareGuideAPIViewSet(BaseAPIViewSet):
    ...
    known_query_parameters = BaseAPIViewSet.known_query_parameters.union(
        ["care_difficulty", "featured", "tag", "plant_type"]
    )
    ...
    def difficulty_levels(self, request):
        ...
        for value, display in choices:
            ...
            results.append({"value": value, "display": display, "count": count})
...
class PlantSpeciesPageViewSet(PagesAPIViewSet):
    ...
    def get_queryset(self):
        ...
        if light_requirements:
            queryset = queryset.filter(
                plant_species__light_requirements=light_requirements
            )

        water_requirements = self.request.GET.get("water_requirements")
        if water_requirements:
            queryset = queryset.filter(
                plant_species__water_requirements=water_requirements
            )
        ...
        ).prefetch_related("categories", "gallery_images")
