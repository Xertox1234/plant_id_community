...
class PlantIdentificationResultViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            PlantIdentificationResult.objects.filter(request__user=self.request.user)
            .annotate(
    ...
    def vote(self, request, pk=None):
        ...
        with transaction.atomic():
            ...
            vote, created = PlantIdentificationVote.objects.get_or_create(
                user=request.user, result=result_obj, defaults={"vote_type": vote_type}
            )
                    ...
                    return Response(
                        {
                            ...
                            "result": self.get_serializer(
                                result_obj, context={"request": request}
                            ).data,
                        }
                    )
                else:
    ...
    def accept(self, request, pk=None):
        ...
        with transaction.atomic():
            ...
            PlantIdentificationResult.objects.filter(request=result_obj.request).update(
                is_accepted=False
            )
        ...
        return Response(
            {"message": "Identification accepted", "result": serializer.data}
        )
    ...
    def add_to_collection(self, request, pk=None):
        ...
        existing_plant = UserPlant.objects.filter(
            collection=collection, from_identification_result=result_obj
        ).first()
        ...
        plant_data = {
            ...
            "care_instructions_json": result_obj.ai_care_instructions
            or {},  # Include AI care instructions
        }
...
def get_care_instructions(request, species_id=None):
    ...
    try:
        if species_id:
            ...
            try:
                species = PlantSpecies.objects.get(
...
def _generate_care_instructions(species: PlantSpecies) -> dict:
    ...
    instructions = {
        ...
        "basic_info": {
            ...
            "family": species.family,
            "plant_type": (
                species.get_plant_type_display() if species.plant_type else "Unknown"
            ),
        },
    }
    ...
    if species.light_requirements:
        ...
        instructions["light"] = light_guide.get(
            species.light_requirements,
            {
    ...
    if species.water_requirements:
        ...
        instructions["watering"] = water_guide.get(
    ...
    if species.mature_height_min or species.mature_height_max:
        ...
        if species.mature_height_min and species.mature_height_max:
            size_info.append(
                f"Mature height: {species.mature_height_min}m - {species.mature_height_max}m"
            )
    ...
    if species.hardiness_zone_min or species.hardiness_zone_max:
        ...
        if species.hardiness_zone_min and species.hardiness_zone_max:
            climate_info.append(
                f"USDA Hardiness Zones: {species.hardiness_zone_min} - {species.hardiness_zone_max}"
            )
        elif species.hardiness_zone_max:
            climate_info.append(
                f"USDA Hardiness Zone up to {species.hardiness_zone_max}"
            )
        elif species.hardiness_zone_min:
            climate_info.append(
                f"USDA Hardiness Zone {species.hardiness_zone_min} and above"
            )
...
class PlantDiseaseResultViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return (
            PlantDiseaseResult.objects.filter(request__user=self.request.user)
    ...
    @action(detail=True, methods=["post"])
    def vote(self, request, pk=None):
        """Vote on a disease diagnosis result with per-user deduplication."""
        result_obj = self.get_object()
        vote_type = request.data.get("vote_type")
        ...
        with transaction.atomic():
            vote, created = PlantDiseaseVote.objects.get_or_create(
                user=request.user,
            ...
            if not created:
                ...
                if previous_vote == vote_type:
                    ...
                    return Response(
                        {
                            "message": f"{vote_type} removed successfully",
                            "result": self.get_serializer(result_obj).data,
                        }
                    )
                else:
                    vote.vote_type = vote_type
    ...
    def accept(self, request, pk=None):
        ...
        with transaction.atomic():
            # Mark all other results as not accepted
            PlantDiseaseResult.objects.filter(request=result_obj.request).update(
                is_accepted=False
            )
        ...
        serializer = self.get_serializer(result_obj)
        return Response(
            {"message": "Disease diagnosis accepted", "result": serializer.data}
        )
    ...
    def save_to_profile(self, request, pk=None):
        ...
        if SavedDiagnosis.objects.filter(
            user=request.user, diagnosis_result=result_obj
        ).exists():
    ...
    def treatment_attempts(self, request, pk=None):
        ...
        attempts = (
            TreatmentAttempt.objects.filter(
                diagnosis_result=result_obj, user=request.user
            )
            .select_related("diagnosis_result", "user")
            .order_by("-created_at")
        )
...
class PlantDiseaseDatabaseViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def disease_types(self, request):
        ...
        types = (
            PlantDiseaseDatabase.objects.values("disease_type")
            .annotate(count=Count("id"), avg_confidence=models.Avg("confidence_score"))
            .order_by("-count")
        )
    ...
    def care_instructions(self, request, pk=None):
        ...
        instructions = DiseaseCareInstructions.objects.filter(disease=disease).order_by(
...
class SavedDiagnosisViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return SavedDiagnosis.objects.filter(user=self.request.user).order_by(
            "-saved_at"
        )
...
class SavedCareInstructionsViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return SavedCareInstructions.objects.filter(user=self.request.user).order_by(
            "-saved_at"
        )
...
class TreatmentAttemptViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return TreatmentAttempt.objects.filter(user=self.request.user).order_by(
            "-created_at"
        )
    ...
    def vote(self, request, pk=None):
        ...
        serializer = self.get_serializer(attempt)
        return Response(
            {"message": "Vote recorded", "treatment_attempt": serializer.data}
        )
...
def search_local_plants(request):
    ...
    try:
        ...
        auto_stored = PlantSpecies.objects.filter(
            auto_stored=True, scientific_name__icontains=query
        ...
        all_plants = (
            ...
            .exclude(id__in=auto_stored.values_list("id", flat=True))
        ...
        serializer = PlantSpeciesSerializer(
            results[:20], many=True, context={"request": request}
        )
...
def search_local_diseases(request):
    ...
    try:
        # Build query
        queryset = PlantDiseaseDatabase.objects.filter(disease_name__icontains=query)
...
def enrich_plant_data(request):
    ...
    try:
        ...
        if not species_data:
            return Response(
                {"error": "Plant species not found in botanical database"}, status=404
            )
        ...
        enriched_data = {
            ...
            "family_description": (
                f"Member of the {main_species.get('family', '')} family"
            ...
            "care_requirements": {
                "light": trefle_service._map_light_requirements(
                    specs.get("light", {}).get("type")
                ),
                "water": trefle_service._map_water_requirements(
                    specs.get("atmospheric_humidity", {}).get("type")
                ),
                "temperature": (
                    f"Hardy in zones {specs.get('minimum_temperature', {}).get('deg_f', {}).get('min', 'N/A')} to {specs.get('maximum_temperature', {}).get('deg_f', {}).get('max', 'N/A')}"
                    if specs.get("minimum_temperature")
                    else ""
                ),
            },
            "native_habitat": ", ".join(
                main_species.get("distribution", {}).get("native", [])
            ),
            "distribution": (
                ", ".join(main_species.get("distribution", {}).get("introduced", []))
                if main_species.get("distribution", {}).get("introduced")
                else ""
            ),
            "interesting_facts": (
                f"This {species_data.get('scientific_name', 'plant')} is native to {', '.join(main_species.get('distribution', {}).get('native', [])[:3])} and belongs to the {main_species.get('family', '')} family."
                if main_species.get("distribution", {}).get("native")
                else ""
            ),
...
def get_plant_characteristics(request, species_id):
    ...
    try:
        ...
        try:
            ...
            characteristics = {
                "mature_height": (
                    f"{species.mature_height_min} - {species.mature_height_max} cm"
                    if species.mature_height_min and species.mature_height_max
                    else ""
                ),
                "mature_width": (
                    f"{species.mature_width_min} - {species.mature_width_max} cm"
                    if species.mature_width_min and species.mature_width_max
                    else ""
                ),
        ...
        characteristics = {
            "mature_height": (
                f"{specs.get('mature_height', {}).get('cm', {}).get('min', 'N/A')} - {specs.get('mature_height', {}).get('cm', {}).get('max', 'N/A')} cm"
                if specs.get("mature_height")
                else ""
            ),
            "mature_width": (
                f"{specs.get('mature_width', {}).get('cm', {}).get('min', 'N/A')} - {specs.get('mature_width', {}).get('cm', {}).get('max', 'N/A')} cm"
                if specs.get("mature_width")
                else ""
            ),
...
def get_plant_growth_info(request, species_id):
    ...
    try:
        ...
        try:
            ...
            growth_info = {
                "light_requirements": (
                    species.get_light_requirements_display()
                    if species.light_requirements
                    else ""
                ),
                "water_requirements": (
                    species.get_water_requirements_display()
                    if species.water_requirements
                    else ""
                ),
                "temperature_range": (
                    f"Hardy zones {species.hardiness_zone_min} - {species.hardiness_zone_max}"
                    if species.hardiness_zone_min and species.hardiness_zone_max
                    else ""
                ),
                "soil_ph": (
                    f"pH {species.soil_ph_min} - {species.soil_ph_max}"
                    if species.soil_ph_min and species.soil_ph_max
                    else ""
                ),
                "source": "local_database",
        ...
        specs = species_data.get("main_species", {}).get("specifications", {})
        growth_info = {
            "light_requirements": trefle_service._map_light_requirements(
                specs.get("light", {}).get("type")
            ),
            "water_requirements": trefle_service._map_water_requirements(
                specs.get("atmospheric_humidity", {}).get("type")
            ),
            "temperature_range": (
                f"Min: {specs.get('minimum_temperature', {}).get('deg_f', {}).get('min', 'N/A')}°F, Max: {specs.get('maximum_temperature', {}).get('deg_f', {}).get('max', 'N/A')}°F"
                if specs.get("minimum_temperature")
                else ""
            ),
            "soil_ph": (
                f"pH {specs.get('soil_ph', {}).get('min', 'N/A')} - {specs.get('soil_ph', {}).get('max', 'N/A')}"
                if specs.get("soil_ph")
                else ""
            ),
            "source": "trefle_api",
        }
...
def regenerate_care_instructions(request, result_id):
    ...
    try:
        ...
        result_obj = get_object_or_404(
            PlantIdentificationResult.objects.select_related(
                "request", "identified_species"
            ),
            id=result_id,
        )
        ...
        scientific_name = result_obj.suggested_scientific_name or (
            result_obj.identified_species.scientific_name
            if result_obj.identified_species
            else ""
        )
        common_names = result_obj.suggested_common_name or (
            result_obj.identified_species.common_names
            if result_obj.identified_species
            else ""
        )
        ...
        if not scientific_name:
            return Response(
                {"error": "No plant name available for care instruction generation"},
                status=400,
            )
        ...
        experience = (
            ...
            if hasattr(request.user, "gardening_experience")
            else "beginner"
        )
        ...
        if not care_instructions:
            return Response(
                {"error": "Failed to generate care instructions"}, status=500
            )

        # Update the result with new care instructions
        ai_care_service.update_result_with_care_instructions(
            result_obj, care_instructions
        )
