...
def backfill_confidence_scores(apps, schema_editor):
    ...
    PlantIdentificationResult = apps.get_model(
        "plant_identification", "PlantIdentificationResult"
    )
    ...
    if null_count > 0:
        print(
            f"[MIGRATION] Backfilled {null_count} NULL confidence_score values with 0.0"
        )
...
def backfill_identification_sources(apps, schema_editor):
    ...
    if total > 0:
        print(
            f"[MIGRATION] Backfilled {total} NULL/empty identification_source values with 'ai_combined'"
        )
...
def reverse_backfill(apps, schema_editor):
    ...
    print(
        "[MIGRATION] Reverse migration: Cannot restore original NULL values (data already migrated)"
    )
