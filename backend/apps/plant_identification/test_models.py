...
class TestPlantIdentificationRequest(TestCase):
    ...
    def test_identification_request_status_choices(self):
        ...
        request = PlantIdentificationRequest.objects.create(
            user=self.user, image_1=self.test_image, status="processing"
        )
...
class TestPlantIdentificationResult(TestCase):
    ...
    def setUp(self):
        ...
        self.identification_request = PlantIdentificationRequest.objects.create(
            user=self.user, image_1=self.create_test_image(), location="Test Garden"
        )
...
class TestPlantDiseaseModels(TestCase):
    ...
    def test_create_disease_request(self):
        ...
        self.assertEqual(
            self.disease_request.symptoms_description, "Yellow leaves with black spots"
        )
...
class TestModelPerformance(TestCase):
    ...
    def setUp(self):
        ...
        for i in range(10):
            plant = PlantSpecies.objects.create(
                scientific_name=f"Test plant {i}", family="Test Family", genus="Test"
            )
    ...
    def create_test_image(self, color="purple"):
        ...
        image_file.seek(0)
        return SimpleUploadedFile(
            name="perf_test.jpg", content=image_file.read(), content_type="image/jpeg"
        )
    ...
    def test_bulk_operations(self):
        ...
        total_requests = PlantIdentificationRequest.objects.filter(
            user=self.user
        ).count()
...
class TestCascadeBehavior(TestCase):
    ...
    def test_user_deletion_cascades_to_identification_requests(self):
        ...
        self.assertTrue(
            PlantIdentificationRequest.objects.filter(id=request_id).exists()
        )
    ...
    def test_species_deletion_preserves_user_plants(self):
        ...
        collection = UserPlantCollection.objects.create(
            user=self.user, name="My Garden", description="Test collection"
        )
    ...
    def test_request_deletion_cascades_to_results(self):
        ...
        self.assertFalse(
            PlantIdentificationResult.objects.filter(id=result_id).exists()
        )
    ...
    def test_cascade_policy_gdpr_compliance(self):
        ...
        collection = UserPlantCollection.objects.create(
            user=self.user, name="My Garden"
        )

        user_plant = UserPlant.objects.create(
            user=self.user, collection=collection, species=self.plant_species
        )
