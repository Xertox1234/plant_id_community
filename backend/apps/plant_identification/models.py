...
from apps.core.validators import (
    validate_image_file,
    validate_plant_identification_image,
)
...
class SavedDiagnosis(models.Model):
    ...
    diagnosis_result = models.ForeignKey(
        PlantDiseaseResult, on_delete=models.CASCADE, related_name="saved_by_users"
    )

    # User's notes and tracking
    personal_notes = models.TextField(
        blank=True, help_text="User's personal notes about this diagnosis and treatment"
    )
    ...
    share_with_community = models.BooleanField(
        default=False, help_text="Share this diagnosis experience with the community"
    )
...
class TreatmentAttempt(models.Model):
    ...
    user_notes = models.TextField(
        blank=True, help_text="User's notes about this treatment attempt"
    )

    side_effects = models.TextField(
        blank=True, help_text="Any negative effects or problems encountered"
    )
...
class PlantDiseaseVote(models.Model):
    """
    Tracks per-user votes on disease diagnosis results to prevent duplicates.
    Mirrors PlantIdentificationVote for the disease diagnosis flow.
    """
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="disease_votes"
    )

    diagnosis_result = models.ForeignKey(
        PlantDiseaseResult, on_delete=models.CASCADE, related_name="user_votes"
    )
    ...
    class Meta:
        unique_together = ["user", "diagnosis_result"]  # One vote per user per result
        ordering = ["-created_at"]

    def __str__(self):
        return f"{self.user.username} voted {self.vote_type} on diagnosis {self.diagnosis_result.id}"
...
class PlantCareBlocks(blocks.StreamBlock):
    ...
    heading = blocks.CharBlock(
        icon="title", template="plant_identification/blocks/heading.html"
    )

    paragraph = blocks.RichTextBlock(
        icon="pilcrow", template="plant_identification/blocks/paragraph.html"
    )
    ...
    care_tip = blocks.StructBlock(
        ...
        icon="help",
        template="plant_identification/blocks/care_tip.html",
    )
    ...
    seasonal_care = blocks.StructBlock(
        ...
        icon="date",
        template="plant_identification/blocks/seasonal_care.html",
    )
    ...
    problem_solution = blocks.StructBlock(
        ...
        icon="warning",
        template="plant_identification/blocks/problem_solution.html",
    )

    image = ImageChooserBlock(
        icon="image", template="plant_identification/blocks/image.html"
    )
    ...
    gallery = blocks.StructBlock(
        ...
        icon="image",
        template="plant_identification/blocks/gallery.html",
    )
...
class PlantCareGuide(models.Model):
    ...
    # Quick care summary
    quick_care_summary = RichTextField(help_text="Brief summary of care requirements")
    ...
    light_description = models.TextField(
        blank=True, help_text="Detailed light requirements description"
    )

    watering_description = models.TextField(
        blank=True, help_text="Detailed watering instructions"
    )

    soil_description = models.TextField(
        blank=True, help_text="Soil requirements and recommendations"
    )

    temperature_description = models.TextField(
        blank=True, help_text="Temperature preferences and tolerance"
    )

    humidity_description = models.TextField(
        blank=True, help_text="Humidity requirements"
    )

    fertilizing_description = models.TextField(
        blank=True, help_text="Fertilizing schedule and recommendations"
    )

    # Propagation information
    propagation_methods = models.TextField(
        blank=True, help_text="How to propagate this plant"
    )
    ...
    seasonal_notes = models.TextField(
        blank=True, help_text="Special seasonal care considerations"
    )

    # Tags for categorization
    tags = ClusterTaggableManager(
        blank=True, help_text="Tags for categorizing care guides"
    )
    ...
    is_featured = models.BooleanField(
        default=False, help_text="Feature this care guide"
    )
    ...
    def care_level_description(self):
        ...
        return dict(self._meta.get_field("care_difficulty").choices)[
            self.care_difficulty
        ]
...
class PlantCategory(models.Model):
    ...
    icon = models.CharField(
        max_length=50, blank=True, help_text="CSS icon class (e.g., 'fas fa-leaf')"
    )

    color = models.CharField(
        max_length=7, default="#28a745", help_text="Category color (hex code)"
    )
    ...
    # Settings
    is_featured = models.BooleanField(
        default=False, help_text="Show this category prominently"
    )

    # Plant species in this category
    plant_species = ParentalManyToManyField(
        PlantSpecies, blank=True, help_text="Plant species in this category"
    )
...
class PlantIdentificationBasePage(Page):
    ...
    meta_description = models.TextField(
        max_length=160, blank=True, help_text="Meta description for search engines"
    )
...
class PlantSpeciesPage(PlantIdentificationBasePage):
    ...
    # Page content
    introduction = RichTextField(help_text="Introduction to this plant species")
    ...
    gallery_images = models.ManyToManyField(
        "wagtailimages.Image", blank=True, help_text="Gallery of plant images"
    )

    # Categorization
    categories = ParentalManyToManyField(
        PlantCategory, blank=True, help_text="Categories for this plant"
    )

    # Settings
    is_featured = models.BooleanField(
        default=False, help_text="Feature this plant species"
    )
...
class PlantCategoryIndexPage(PlantIdentificationBasePage):
    ...
    introduction = RichTextField(
        blank=True, help_text="Introduction to plant categories"
    )

    categories_per_page = models.IntegerField(
        default=12, help_text="Number of categories to display per page"
    )

    show_featured_plants = models.BooleanField(
        default=True, help_text="Show featured plants section"
    )
...
class BatchIdentificationRequest(models.Model):
    ...
    description = models.TextField(
        blank=True, help_text="Optional description of the batch identification session"
    )
    ...
    total_images = models.PositiveIntegerField(
        default=0, help_text="Total number of images in this batch"
    )
    ...
    failed_images = models.PositiveIntegerField(
        default=0, help_text="Number of images that failed processing"
    )

    # AI Processing
    processing_started_at = models.DateTimeField(
        null=True, blank=True, help_text="When AI processing started"
    )

    processing_completed_at = models.DateTimeField(
        null=True, blank=True, help_text="When AI processing completed"
    )
    ...
    auto_accept_high_confidence = models.BooleanField(
        default=False, help_text="Automatically accept results with >90% confidence"
    )

    share_with_community = models.BooleanField(
        default=False, help_text="Share this batch with the community"
    )
...
class BatchIdentificationComparison(models.Model):
    ...
    batch_request = models.ForeignKey(
        BatchIdentificationRequest, on_delete=models.CASCADE, related_name="comparisons"
    )
    ...
    comparison_title = models.CharField(
        max_length=200, blank=True, help_text="Title for this comparison group"
    )

    comparison_notes = models.TextField(
        blank=True, help_text="User notes about the comparison"
    )
    ...
    decision_reasoning = models.TextField(
        blank=True, help_text="User's reasoning for their selection"
    )
    ...
    completed_at = models.DateTimeField(
        null=True, blank=True, help_text="When user completed this comparison"
    )
...
class BatchIdentificationImage(models.Model):
    ...
    latitude = models.FloatField(
        null=True, blank=True, help_text="GPS latitude if available in EXIF data"
    )

    longitude = models.FloatField(
        null=True, blank=True, help_text="GPS longitude if available in EXIF data"
    )
    ...
    quality_issues = models.JSONField(
        default=list, help_text="List of detected quality issues (blur, lighting, etc.)"
    )
    ...
    user_notes = models.TextField(
        blank=True, help_text="User's notes about this specific plant/image"
    )

    # Timestamps
    uploaded_at = models.DateTimeField(auto_now_add=True)
    processed_at = models.DateTimeField(
        null=True, blank=True, help_text="When processing completed for this image"
    )
    ...
    def __str__(self):
        return (
            f"Image {self.upload_order} in batch {self.batch_request.batch_id.hex[:8]}"
        )
...
class BatchProcessingQueue(models.Model):
    ...
    max_concurrent_requests = models.PositiveIntegerField(
        default=3, help_text="Maximum number of concurrent API requests for this batch"
    )
    ...
    def complete_processing(self, success=True, error_message=""):
        ...
        if success:
            ...
            if self.retry_count < self.max_retries:
                ...
                self.queue_position = (
                    BatchProcessingQueue.objects.filter(status="queued").count() + 1
                )
