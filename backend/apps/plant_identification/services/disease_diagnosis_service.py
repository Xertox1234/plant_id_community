...
class DiseaseAutostorageService:
    ...
    def store_disease_if_qualified(
        self, result: "PlantDiseaseResult"
    ) -> Optional["PlantDiseaseDatabase"]:
        """
    ...
    def _update_existing_disease(
        self, disease: "PlantDiseaseDatabase", result: "PlantDiseaseResult"
    ) -> "PlantDiseaseDatabase":
                ...
                if isinstance(treatment_list, list):
                    for treatment_text in treatment_list:
                        treatment_methods.append(
                            {"type": treatment_type, "instructions": treatment_text}
                        )
        ...
        for treatment in treatment_methods:
            try:
                ...
                existing = DiseaseCareInstructions.objects.filter(
                    disease=disease, treatment_name__icontains=treatment["type"]
                ).first()
...
class PlantDiseaseService:
    ...
    def diagnose_disease_from_request(
        ...
        try:
            ...
            if request.symptoms_description:
                if progress_cb:
                    progress_cb(
                        "local_search_start",
                        "processing",
                        {"query": request.symptoms_description[:50]},
                    )
                ...
                if progress_cb:
                    progress_cb(
                        "local_search_done",
                        "processing",
                        {"results": len(local_results)},
                    )

            # Step 2: Decide if we need API call based on local results
            use_api = True
            if self.plant_health and local_results:
                use_api = self.plant_health.should_use_api(
                    local_results, min_local_confidence=0.7
                )
            ...
            if use_api and self.plant_health:
                if progress_cb:
                    progress_cb(
                        "api_diagnosis_start", "processing", {"images": len(images)}
                    )
                ...
                if progress_cb:
                    progress_cb(
                        "api_diagnosis_done",
                        "processing",
                        {"results": len(api_results)},
                    )
            else:
                # Use local database results
                results.extend(
                    self._create_results_from_local_data(request, local_results)
                )
                if progress_cb:
                    progress_cb(
                        "local_results_used", "processing", {"results": len(results)}
                    )

            # Step 4: Auto-store high-confidence results
            if progress_cb:
                progress_cb(
                    "auto_storage_start", "processing", {"results": len(results)}
                )
            ...
            if progress_cb:
                progress_cb(
                    "auto_storage_done",
                    "processing",
                    {"stored": len([r for r in results if r.stored_to_database])},
                )
    ...
    def _search_local_database(self, request: PlantDiseaseRequest) -> List[Dict]:
        ...
        return self.plant_health.search_local_database(
            disease_name=request.symptoms_description, disease_type=None
        )

    def _diagnose_with_api(
        self, request: PlantDiseaseRequest, images: List
    ) -> List[PlantDiseaseResult]:
        ...
        try:
            ...
            # Get top suggestions
            suggestions = self.plant_health.get_top_disease_suggestions(
                diagnosis, min_probability=0.1
            )
    ...
    def _create_results_from_local_data(
        self, request: PlantDiseaseRequest, local_results: List[Dict]
    ) -> List[PlantDiseaseResult]:
    ...
    def _generate_immediate_actions(self, normalized_data: Dict) -> str:
        ...
        if confidence >= 0.7:
            ...
            actions.append(
                "Moderate confidence diagnosis - monitor symptoms and begin treatment."
            )
        else:
            actions.append(
                "Low confidence diagnosis - consider getting a second opinion."
            )
    ...
    def _create_fallback_result(self, request: PlantDiseaseRequest):
        ...
        try:
            ...
            if request.symptoms_description:
                notes += (
                    f"Based on your description: '{request.symptoms_description}', "
                )
    ...
    def get_service_status(self) -> Dict:
        ...
        status["combined_service"]["available"] = status["plant_health"].get(
            "status"
        ) == "available" or status["local_database"].get("available", False)
