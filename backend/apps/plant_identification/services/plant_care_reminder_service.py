...
class PlantCareReminderService:
    ...
    def create_reminder_from_care_instructions(
        ...
        try:
            ...
            care_instructions = SavedCareInstructions.objects.get(
                id=saved_care_instructions_id, user=user
            )
        except SavedCareInstructions.DoesNotExist:
            raise ValueError(
                f"Care instructions {saved_care_instructions_id} not found for user {user.username}"
            )
    ...
    def get_due_reminders(self, as_of: datetime = None) -> List[PlantCareReminder]:
        ...
        return PlantCareReminder.objects.filter(
            is_active=True, next_reminder_date__lte=as_of
    ...
    def get_user_reminders(
        ...
        return queryset.select_related("saved_care_instructions").order_by(
            "next_reminder_date"
        )

    def update_reminder_frequency(
        self, reminder: PlantCareReminder, new_frequency: str
    ) -> bool:
        """
    ...
    def _build_care_context(self, reminder: PlantCareReminder) -> Dict[str, Any]:
        ...
        if reminder.saved_care_instructions:
            try:
                ...
                context["care_instructions"] = (
                    reminder.custom_instructions or "Care for your plant as needed."
                )
        else:
            context["care_instructions"] = (
                reminder.custom_instructions or "Care for your plant as needed."
        ...
        context["plant_tips"] = self._get_care_tips(
            reminder.plant_scientific_name, reminder.care_type
        )
    ...
    def _extract_care_instructions(self, care_data: Dict, care_type: str) -> str:
        ...
        return care_data.get(
            "care_summary", "Care for your plant according to its needs."
        )
    ...
    def _build_care_instructions_url(self, reminder: PlantCareReminder) -> str:
        ...
        if reminder.saved_care_instructions:
            return (
                f"/profile/care-instructions/{reminder.saved_care_instructions.uuid}/"
            )
