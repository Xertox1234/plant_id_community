...
class APIMonitoringService:
    ...
    def record_api_call(
        self, api_name: str, endpoint: str = None, success: bool = True
    ):
        """
        ...
        now = timezone.now()
        hour_key = (
            f"{self.CACHE_KEYS[f'{api_name}_calls_hour']}:{now.strftime('%Y%m%d%H')}"
        )
    ...
    def record_identification_request(self, source: str = "web"):
        ...
        now = timezone.now()
        day_key = (
            f"{self.CACHE_KEYS['identification_requests']}:{now.strftime('%Y%m%d')}"
        )
    ...
    def get_api_usage(self, api_name: str) -> Dict:
        ...
        usage = {
            ...
            "hourly_percentage": (
                (hourly_calls / limits.get("hourly", 1)) * 100
                if limits.get("hourly")
                else 0
            ),
            "daily_calls": daily_calls,
            "daily_limit": limits.get("daily", 0),
            "daily_percentage": (
                (daily_calls / limits.get("daily", 1)) * 100
                if limits.get("daily")
                else 0
            ),
    ...
    def get_system_health(self) -> Dict:
        ...
        cache_healthy = (
            cache_perf["cache_hit_ratio"] > 50
        )  # At least 50% cache hit rate
        ...
        return {
            "overall_health": (
                "healthy" if all_apis_healthy and cache_healthy else "degraded"
            ),
            "apis": api_status,
            "cache_performance": {
    ...
    def _get_health_recommendations(
        self, api_status: Dict, cache_perf: Dict
    ) -> List[str]:
        ...
        for api_name, status in api_status.items():
            if status["alert_level"] == "critical":
                recommendations.append(
                    f"Immediate action required: {api_name} API usage critical"
                )
        ...
        if cache_perf["local_db_ratio"] < 30:
            recommendations.append(
                "Increase local database coverage with common species"
            )
