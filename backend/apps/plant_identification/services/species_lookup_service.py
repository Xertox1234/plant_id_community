...
class SpeciesLookupService:
    ...
    def search_species_by_common_name(
        self, common_name: str, limit: int = 10
    ) -> List[Dict]:
        """
    ...
    def _get_local_species_fallback(
    ...
    def _search_local_species(self, query: str, limit: int = 10) -> List[Dict]:
        ...
        try:
            ...
            for species in species_matches:
                confidence_type = (
                    "local_verified" if species.expert_reviewed else "local_frequent"
                )
    ...
    def _cache_species_data(
        self, scientific_name: str, data: Dict, timeout: int = 86400
    ) -> None:
            ...
            if species_data:
                ...
                if detailed_data:
                    normalized_data = self.trefle.normalize_plant_data(
                        {"main_species": detailed_data}
                    )
                    normalized_data["confidence_score"] = self.CONFIDENCE_THRESHOLDS[
                        "fresh_api"
                    ]
    ...
    def _format_local_species(
        self, species: PlantSpecies, confidence_type: str
    ) -> Dict:
    ...
    def get_popular_species(self, limit: int = 20) -> List[PlantSpecies]:
        ...
        return PlantSpecies.objects.filter(identification_count__gt=0).order_by(
            "-identification_count", "-confidence_score"
        )[:limit]
