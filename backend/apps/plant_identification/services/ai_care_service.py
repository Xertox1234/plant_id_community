...
class AIPlantCareService:
    ...
    def generate_care_instructions(
        ...
        try:
            ...
            prompt = self._create_enhanced_care_prompt(
                plant_name,
                common_names,
                location,
                climate,
        ...
        if botanical_data:
            ...
            if plantnet_data.get("iucn_category"):
                context_parts.append(
                    f"Conservation status: {plantnet_data['iucn_category']}"
                )
            if plantnet_data.get("growth_form"):
                context_parts.append(
                    f"Natural growth form: {plantnet_data['growth_form']}"
                )
            ...
            if trefle_data.get("light_requirements"):
                context_parts.append(
                    f"Light preference: {trefle_data['light_requirements']}"
                )
            if trefle_data.get("water_requirements"):
                context_parts.append(
                    f"Water needs: {trefle_data['water_requirements']}"
                )
            if trefle_data.get("mature_height_min") and trefle_data.get(
                "mature_height_max"
            ):
                context_parts.append(
                    f"Mature size: {trefle_data['mature_height_min']}-{trefle_data['mature_height_max']}cm"
                )
            if trefle_data.get("hardiness_zone_min") and trefle_data.get(
                "hardiness_zone_max"
            ):
                context_parts.append(
                    f"Hardiness zones: {trefle_data['hardiness_zone_min']}-{trefle_data['hardiness_zone_max']}"
                )
            ...
            if trefle_data.get("soil_ph_min") and trefle_data.get("soil_ph_max"):
                context_parts.append(
                    f"Soil pH preference: {trefle_data['soil_ph_min']}-{trefle_data['soil_ph_max']}"
                )
...
Ensure all advice is scientifically accurate, tailored for {experience_level} level, and specifically adapted for {location} climate conditions using the botanical data provided. Prioritize evidence-based recommendations over generic advice."""
    ...
    def _generate_fallback_instructions(
        self, plant_name: str, common_names: str
    ) -> Dict:
        """
        ...
        """
        display_name = (
            common_names.split(",")[0].strip() if common_names else plant_name
        )
        ...
        return {
            ...
            "special_care": [
                "Observe your plant and adjust care based on its response"
            ],
            "companion_plants": [],
            "fun_facts": [
                f"{display_name} is a wonderful addition to any plant collection!"
            ],
