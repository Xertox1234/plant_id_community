...
def rate_limit(max_calls_per_hour=120):
    ...
    def decorator(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            current_calls = [
                call_time for call_time in current_calls if now - call_time < 3600
            ]
...
class TrefleAPIService:
    ...
    def _make_request(
        ...
        try:
            response = self.session.get(
                url, params=params or {}, timeout=self.REQUEST_TIMEOUT
            )
    ...
    def normalize_plant_data(self, trefle_data: Dict) -> Dict:
        ...
        return {
            ...
            "common_names": ", ".join(
                trefle_data.get("common_names", {}).get("en", [])
            ),
            "family": main_species.get("family", ""),
            "genus": main_species.get("genus", ""),
            "plant_type": specs.get("growth_form", {}).get("type", ""),
            "mature_height_min": specs.get("mature_height", {})
            .get("cm", {})
            .get("min"),
            "mature_height_max": specs.get("mature_height", {})
            .get("cm", {})
            .get("max"),
            "light_requirements": self._map_light_requirements(
                specs.get("light", {}).get("type")
            ),
            "water_requirements": self._map_water_requirements(
                specs.get("atmospheric_humidity", {}).get("type")
            ),
            "soil_ph_min": specs.get("soil_ph", {}).get("min"),
            "soil_ph_max": specs.get("soil_ph", {}).get("max"),
            "hardiness_zone_min": specs.get("minimum_temperature", {})
            .get("deg_f", {})
            .get("min"),
            "hardiness_zone_max": specs.get("maximum_temperature", {})
            .get("deg_f", {})
            .get("max"),
