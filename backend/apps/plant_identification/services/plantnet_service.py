...
class PlantNetAPIService:
    ...
    def _call_plantnet_api(
        ...
        try:
            ...
            logger.info(
                f"[CACHE] Stored PlantNet result for image {image_hash[:8]}... (TTL: {self.CACHE_TIMEOUT}s)"
            )
    ...
    def identify_plant(
        ...
        try:
            ...
            if cached_result:
                logger.info(
                    f"[CACHE] HIT for PlantNet image {image_hash[:8]}... (instant response)"
                )
                return cached_result

            logger.info(
                f"[CACHE] MISS for PlantNet image {image_hash[:8]}... (checking quota)"
            )
            ...
            if not self.quota_manager.can_call_plantnet():
                ...
                logger.error(
                    f"[QUOTA] PlantNet hourly quota EXHAUSTED ({hourly_usage}/20 used)"
                )
                raise QuotaExceeded(
            ...
            result = self.circuit.call(
                self._call_plantnet_api, url, params, files, data, cache_key, image_hash
            )
    ...
    def get_top_suggestions(
        self, identification_result: Dict[str, Any], min_score: float = 0.1
    ) -> List[Dict[str, Any]]:
        """
                ...
                suggestions.append(
                    {
                        ...
                        "gbif_id": result.get("gbif", {}).get(
                            "id", ""
                        ),  # GBIF ID is at result level
                        "powo_id": result.get("powo", {}).get(
                            "id", ""
                        ),  # POWO ID is at result level
                        "iucn_id": result.get("iucn", {}).get(
                            "id", ""
                        ),  # IUCN ID is at result level
                        "iucn_category": result.get("iucn", {}).get(
                            "category", ""
                        ),  # Conservation status
                    }
                )
    ...
    def get_all_projects(self) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            response = self.session.get(
                url, params=params, timeout=IMAGE_DOWNLOAD_TIMEOUT
            )
            response.raise_for_status()
    ...
    def get_service_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.session.get(
                url, params=params, timeout=IMAGE_DOWNLOAD_QUICK_TIMEOUT
            )
