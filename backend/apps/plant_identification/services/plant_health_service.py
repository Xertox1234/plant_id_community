...
class PlantHealthAPIService:
    ...
    def get_top_disease_suggestions(
        self, diagnosis_result: Dict, min_probability: float = 0.1
    ) -> List[Dict]:
        """
        ...
        for disease_data in health_assessment["diseases"]:
            ...
            if probability >= min_probability:
                ...
                suggestion = {
                    ...
                    "disease_type": self._categorize_disease(
    ...
    def _categorize_disease(self, disease_name: str) -> str:
        ...
        if any(
            term in disease_name_lower
            for term in ["mold", "mildew", "rust", "blight", "fungus", "rot", "canker"]
        ):
            return "fungal"
        ...
        if any(
            term in disease_name_lower
            for term in ["bacterial", "bacteria", "fire blight", "soft rot"]
        ):
            return "bacterial"
        ...
        if any(
            term in disease_name_lower
            for term in ["virus", "viral", "mosaic", "yellows"]
        ):
            return "viral"
        ...
        if any(
            term in disease_name_lower
            for term in ["burn", "scorch", "deficiency", "toxicity", "stress"]
        ):
            return "abiotic"
    ...
    def normalize_disease_data(self, suggestion: Dict) -> Dict:
        ...
        return {
            ...
            "prevention_tips": (
                prevention_info.get("description", "")
                if isinstance(prevention_info, dict)
                else str(prevention_info)
            ),
            "severity_assessment": self._assess_severity(
                suggestion.get("probability", 0.0)
            ),
    ...
    def get_service_status(self) -> Dict:
        ...
        try:
            ...
            # This dict is a response shape: it is merged into
            # DiseaseDiagnosisService.get_service_status and returned. `str(e)`
            # on a requests exception embeds the prepared URL, i.e. the api-key
            # (todos 354/358/377), so the detail goes to the log and the caller
            # gets a constant.
            if isinstance(e, requests.RequestException):
                logger.error(
                    "[PLANT_HEALTH] Service status check failed: %s",
                    log_safe_api_error(e),
                )
            else:
                logger.exception("[PLANT_HEALTH] Service status check failed")
            return {
                "status": "error",
                "api_key_valid": False,
    ...
    def search_local_database(
        self, disease_name: str, disease_type: Optional[str] = None
    ) -> List[Dict]:
        """
    ...
    def should_use_api(
        self, local_results: List[Dict], min_local_confidence: float = 0.7
    ) -> bool:
        """
