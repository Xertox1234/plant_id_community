...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f"Testing plant identification APIs..."))
        ...
        try:
            ...
            if plants:
                ...
                for i, plant in enumerate(plants, 1):
                    ...
                    self.stdout.write(
                        f"   Common names: {plant.get('common_names', {}).get('en', [])}"
                    )
                ...
                if plant_id:
                    ...
                    if details:
                        ...
                        self.stdout.write(
                            f"Family: {main_species.get('family', 'Unknown')}"
                        )
                        self.stdout.write(
                            f"Genus: {main_species.get('genus', 'Unknown')}"
                        )
            else:
                self.stdout.write("No plants found")

        except Exception as e:
            self.stdout.write(self.style.ERROR(f"Trefle API test failed: {str(e)}"))
        ...
        try:
            ...
            if projects:
                ...
                for project in projects[:3]:  # Show first 3
                    ...
                    self.stdout.write(
                        f"  Species: {project['species_count']}, Images: {project['image_count']}"
                    )
            ...
            self.stdout.write(self.style.ERROR(f"PlantNet API test failed: {str(e)}"))
        ...
        try:
            ...
            for service_name, service_status in status.items():
                ...
                if status_text == "available":
                    self.stdout.write(
                        f"  {service_name}: " + self.style.SUCCESS("✓ Available")
                    )
                else:
                    self.stdout.write(
                        f"  {service_name}: " + self.style.ERROR("✗ Not Available")
                    )
