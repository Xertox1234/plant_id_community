...
class SearchQuery(models.Model):
    ...
    response_time_ms = models.IntegerField(
        null=True, blank=True, help_text="Search response time in milliseconds"
    )

    # Applied filters (JSON field)
    filters_applied = models.JSONField(
        default=dict, blank=True, help_text="Filters that were applied to the search"
    )
...
class UserSearchPreferences(models.Model):
    ...
    results_per_page = models.IntegerField(
        default=20, help_text="Number of results to show per page"
    )

    # Search history settings
    save_search_history = models.BooleanField(
        default=True, help_text="Whether to save search history"
    )

    search_history_days = models.IntegerField(
        default=30, help_text="Number of days to keep search history"
    )

    # Personalization settings
    enable_personalized_results = models.BooleanField(
        default=True, help_text="Enable personalized search results based on interests"
    )

    boost_expert_content = models.BooleanField(
        default=True, help_text="Boost results from expert users"
    )

    # Notification preferences
    enable_search_alerts = models.BooleanField(
        default=False, help_text="Enable alerts for new content matching saved searches"
    )
...
class SavedSearch(models.Model):
    ...
    name = models.CharField(
        max_length=100, help_text="User-defined name for the saved search"
    )

    query_text = models.TextField(help_text="The search query text")

    content_type = models.CharField(
        max_length=20, choices=SearchQuery.CONTENT_TYPE_CHOICES, default="all"
    )

    # Search parameters (JSON field for filters, sorting, etc.)
    search_parameters = models.JSONField(
        default=dict, help_text="Complete search parameters including filters"
    )

    # Alert settings
    enable_alerts = models.BooleanField(
        default=False, help_text="Send alerts when new content matches this search"
    )
    ...
    last_alert_sent = models.DateTimeField(
        null=True, blank=True, help_text="When the last alert was sent"
    )
    ...
    times_used = models.IntegerField(
        default=0, help_text="Number of times this saved search has been used"
    )

    last_used = models.DateTimeField(
        null=True, blank=True, help_text="When this saved search was last used"
    )
...
class SearchResultClick(models.Model):
    ...
    search_query = models.ForeignKey(
        SearchQuery, on_delete=models.CASCADE, related_name="result_clicks"
    )

    # Result metadata
    result_type = models.CharField(
        max_length=20, help_text="Type of content that was clicked"
    )

    result_id = models.CharField(
        max_length=100, help_text="ID of the content that was clicked"
    )
    ...
    result_page = models.IntegerField(
        default=1, help_text="Page number where the result appeared"
    )
    ...
    def __str__(self):
        return (
            f"Click on {self.result_type}:{self.result_id} (pos {self.result_position})"
        )
