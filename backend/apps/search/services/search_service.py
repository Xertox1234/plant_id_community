...
from apps.plant_identification.models import PlantDiseaseDatabase, PlantSpecies
...
class UnifiedSearchService:
    ...
    def search(
        ...
        try:
            ...
            if "forum" in content_types:
                forum_results, forum_count = self._search_forum(
            ...
            if "plants" in content_types:
                plant_results, plant_count = self._search_plants(
                    search_query, query, filters
                )
            ...
            if "blog" in content_types:
                blog_results, blog_count = self._search_blog(
                    search_query, query, filters
                )
            ...
            if "diseases" in content_types:
                disease_results, disease_count = self._search_diseases(
                    search_query, query, filters
                )
    ...
    def _search_forum(
        self, search_query, query_text: str, filters: Dict
    ) -> Tuple[List[Dict], int]:
        ...
        topics_qs = (
            topics_qs.annotate(
                search=SearchVector("subject", weight="A")
        ...
        for topic in topics_qs.select_related("poster", "forum")[
            :20
        ]:  # Limit per content type
            results.append(
                {
                    "type": "forum_topic",
                    "id": str(topic.id),
                    "title": topic.subject,
                    "content": (
                        getattr(topic.first_post, "content", "")[:200] + "..."
                        if getattr(topic.first_post, "content", "")
                        else ""
                    ),
                    "url": f"/forum/topic/{topic.id}",
                    "author": {
                        "username": (
                            topic.poster.username if topic.poster else "Unknown"
                        ),
                    ...
                    "metadata": {
                        ...
                        "replies_count": getattr(topic, "posts_count", 0)
                        - 1,  # Subtract first post
            ...
            results.append(
                {
                    ...
                    "created_at": (
                        post.first_published_at.isoformat()
    ...
    def _search_diseases(
        ...
        if filters.get("affected_plants"):
            diseases_qs = diseases_qs.filter(
                affected_plants__icontains=filters["affected_plants"]
            )

        # Apply full-text search
        diseases_qs = (
            diseases_qs.annotate(
            ...
            results.append(
                {
                    ...
                    "content": (
                        disease.description[:200] + "..." if disease.description else ""
                    ),
                    "url": f"/diseases/{disease.uuid}",
                    "author": None,  # Diseases don't have authors
                    "created_at": (
                        disease.created_at.isoformat()
                        if hasattr(disease, "created_at")
                        else None
                    ),
                    "metadata": {
                        ...
                        "symptoms": (
                            disease.symptoms[:100] + "..." if disease.symptoms else ""
                        ),
                    },
    ...
    def _sort_results(self, results: List[Dict], sort_by: str) -> List[Dict]:
        ...
        if sort_by == "relevance":
            ...
            return sorted(
                results,
                key=lambda x: x.get("metadata", {}).get("views_count", 0),
                reverse=True,
            )
        else:
            return results
    ...
    def get_search_suggestions(self, partial_query: str, limit: int = 10) -> List[Dict]:
        ...
        recent_searches = (
            SearchQuery.objects.filter(query_text__icontains=partial_query)
            .values_list("query_text", flat=True)
            .distinct()[: limit // 2]
        )

        for query in recent_searches:
            suggestions.append({"text": query, "type": "recent_search", "score": 1.0})
        ...
        for name in plant_names:
            suggestions.append({"text": name, "type": "plant_name", "score": 0.8})
