...
from .models import SavedSearch, SearchQuery, UserSearchPreferences
...
class SearchRequestSerializer(serializers.Serializer):
    ...
    query = serializers.CharField(
        min_length=2, max_length=500, help_text="Search query text"
    )
    ...
    per_page = serializers.IntegerField(
        min_value=1, max_value=100, default=20, help_text="Number of results per page"
    )

    sort_by = serializers.ChoiceField(
        choices=SORT_CHOICES, default="relevance", help_text="How to sort the results"
    )

    # Filter parameters
    date_from = serializers.DateTimeField(
        required=False, help_text="Filter results from this date"
    )

    date_to = serializers.DateTimeField(
        required=False, help_text="Filter results until this date"
    )

    forum_category = serializers.IntegerField(
        required=False, help_text="Filter forum results by category ID"
    )

    blog_category = serializers.CharField(
        required=False, help_text="Filter blog results by category slug"
    )

    plant_family = serializers.CharField(
        required=False, help_text="Filter plant results by family"
    )

    plant_type = serializers.CharField(
        required=False, help_text="Filter plant results by type"
    )

    care_level = serializers.CharField(
        required=False, help_text="Filter plant results by care difficulty"
    )

    disease_type = serializers.CharField(
        required=False, help_text="Filter disease results by type"
    )
    ...
    def validate(self, data):
        ...
        if (
            "all" in data.get("content_types", [])
            and len(data.get("content_types", [])) > 1
        ):
...
class SearchResultSerializer(serializers.Serializer):
    ...
    author = serializers.DictField(
        required=False, allow_null=True, help_text="Author information"
    )

    created_at = serializers.CharField(
        required=False, allow_null=True, help_text="Creation date (ISO format)"
    )
    ...
    rank = serializers.FloatField(help_text="Search relevance rank")
...
class SearchResponseSerializer(serializers.Serializer):
    ...
    content_types_searched = serializers.ListField(
        help_text="Content types that were searched"
    )

    error = serializers.CharField(
        required=False, help_text="Error message if search failed"
    )
...
class SearchSuggestionsRequestSerializer(serializers.Serializer):
    ...
    query = serializers.CharField(
        min_length=1, max_length=100, help_text="Partial search query"
    )

    limit = serializers.IntegerField(
        min_value=1, max_value=20, default=10, help_text="Maximum number of suggestions"
    )
...
class UserSearchPreferencesSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "save_search_history",
            "search_history_days",
            "enable_personalized_results",
            "boost_expert_content",
    ...
    def validate_results_per_page(self, value):
        ...
        if value < 5 or value > 100:
            raise serializers.ValidationError(
                "Results per page must be between 5 and 100."
            )
        return value
    ...
    def validate_search_history_days(self, value):
        ...
        if value < 1 or value > 365:
            raise serializers.ValidationError(
                "Search history days must be between 1 and 365."
            )
        return value
...
class SearchAnalyticsSerializer(serializers.Serializer):
    ...
    content_type_distribution = serializers.DictField(
        help_text="Search distribution by content type"
    )
    search_trends = serializers.ListField(help_text="Search volume trends over time")
    zero_result_queries = serializers.ListField(
        help_text="Queries that returned no results"
    )
