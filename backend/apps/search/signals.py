...
def update_topic_search_vector(sender, instance, **kwargs):
    ...
    try:
        ...
        logger.error(
            f"Failed to update search vector for topic {instance.id}: {str(e)}"
        )
...
def update_plant_search_vector(sender, instance, **kwargs):
    ...
    try:
        ...
        logger.error(
            f"Failed to update search vector for plant species {instance.id}: {str(e)}"
        )
...
def update_disease_search_vector(sender, instance, **kwargs):
    ...
    try:
        ...
        logger.error(
            f"Failed to update search vector for disease {instance.id}: {str(e)}"
        )
...
def update_blog_search_vector(sender, instance, **kwargs):
    ...
    try:
        ...
        logger.error(
            f"Failed to update search vector for blog post {instance.id}: {str(e)}"
        )
...
def clear_plant_search_vector(sender, instance, **kwargs):
    ...
    logger.debug(
        f"Plant species {instance.id} deleted, search vector automatically cleared"
    )
...
def clear_blog_search_vector(sender, instance, **kwargs):
    ...
    logger.debug(
        f"Blog post {instance.id} deleted, search vector automatically cleared"
    )
