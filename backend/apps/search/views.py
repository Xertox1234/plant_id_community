    ...
    def post(self, request):
        ...
        try:
            ...
            if response_serializer.is_valid():
                ...
                logger.error(
                    f"Search response serialization failed: {response_serializer.errors}"
                )
                return Response(
                    search_results, status=status.HTTP_200_OK
        ...
        try:
            ...
            try:
                ...
                plant_types = [
                    ...
                    for choice in plant_type_choices
                ]
            except Exception:
                pass
            ...
            care_levels = []
            try:
                care_level_choices = PlantSpecies._meta.get_field(
                    "care_difficulty"
                ).choices
                care_levels = [
                    {"value": choice[0], "label": choice[1]}
                    for choice in care_level_choices
                ]
            except Exception:
                pass
            ...
            disease_types = []
            try:
                disease_type_choices = PlantDiseaseDatabase._meta.get_field(
                    "disease_type"
                ).choices
                disease_types = [
                    {"value": choice[0], "label": choice[1]}
                    for choice in disease_type_choices
                ]
            except Exception:
                pass
            ...
            if serializer.is_valid():
                ...
                return Response(
                    filters_data, status=status.HTTP_200_OK
                )  # Return raw data if serialization fails
...
class SearchAnalyticsView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            avg_response_time = (
                searches.aggregate(avg_time=Avg("response_time_ms"))["avg_time"] or 0
            )
            ...
            content_type_dist = {}
            for search in searches.values("content_type").annotate(
            ...
            for i in range(days):
                ...
                search_trends.append(
                    {"date": day.date().isoformat(), "count": day_count}
                )
            ...
            if serializer.is_valid():
                ...
                return Response(
                    analytics_data, status=status.HTTP_200_OK
                )  # Return raw data if serialization fails
...
def track_search_click(request):
    ...
    try:
        ...
        try:
            ...
            return Response(
                {"error": "Search query not found"}, status=status.HTTP_404_NOT_FOUND
            )
