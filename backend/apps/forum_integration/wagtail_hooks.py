...
def register_forum_menu():
    ...
    """
    return MenuItem("Forum Management", "/forum/", icon_name="group", order=300)
...
def forum_page_listing_buttons(
    ...
    if isinstance(page, (ForumIndexPage, ForumCategoryPage)):
        yield wagtailadmin_widgets.ListingButton(
            "View Forum Stats", "/forum/", icon_name="view", priority=10
        )
    ...
    if isinstance(page, ForumCategoryPage) and page.machina_forum_id:
        try:
            ...
            yield wagtailadmin_widgets.ListingButton(
                "View Live Forum", forum_url, icon_name="link-external", priority=20
            )
...
def forum_page_listing_more_buttons(
    page, page_perms=None, user=None, is_parent=False, next_url=None, **kwargs
):
    """
    ...
    if isinstance(page, ForumCategoryPage):
        yield wagtailadmin_widgets.Button(
            "Moderate", "/forum/moderation/", icon_name="warning", priority=30
        )
...
def hide_unused_menu_items(request, menu_items):
    ...
    if not request.user.is_superuser:
        ...
        menu_items[:] = [
            item for item in menu_items if item.name not in ["settings", "help"]
        ]
...
def register_forum_settings_menu_item():
    ...
    """
    return MenuItem("Forum Settings", "/forum/", icon_name="cog", order=500)
...
def register_forum_search():
    ...
    """
    return SearchArea(
        "Forum Content", "/forum/search/", name="forum", icon_name="group", order=400
    )
...
def register_forum_permissions():
    ...
    return Permission.objects.filter(
        ...
        codename__in=[
            "add_forumpagemapping",
            "change_forumpagemapping",
            "delete_forumpagemapping",
        ],
    )
