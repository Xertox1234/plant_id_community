...
class ForumStreamBlocks(blocks.StreamBlock):
    ...
    heading = blocks.CharBlock(
        icon="title", template="forum_integration/blocks/heading.html"
    )

    paragraph = blocks.RichTextBlock(
        icon="pilcrow", template="forum_integration/blocks/paragraph.html"
    )
    ...
    forum_rules = blocks.StructBlock(
        ...
        icon="list-ul",
        template="forum_integration/blocks/rules.html",
    )
    ...
    moderator_info = blocks.StructBlock(
        ...
        icon="user",
        template="forum_integration/blocks/moderator.html",
    )

    image = ImageChooserBlock(
        icon="image", template="forum_integration/blocks/image.html"
    )
    ...
    call_to_action = blocks.StructBlock(
        ...
        icon="link",
        template="forum_integration/blocks/cta.html",
    )
    ...
    statistics = blocks.StructBlock(
        ...
        icon="view",
        template="forum_integration/blocks/statistics.html",
    )
...
class ForumBasePage(Page):
    ...
    enable_social_sharing = models.BooleanField(
        default=True, help_text="Enable social media sharing buttons"
    )
...
class ForumIndexPage(ForumBasePage):
    ...
    forums_per_page = models.IntegerField(
        default=20, help_text="Number of forums to display per page"
    )

    show_statistics = models.BooleanField(
        default=True, help_text="Show forum statistics (total posts, topics, etc.)"
    )

    welcome_message = models.TextField(
        blank=True, help_text="Welcome message displayed at the top of the forum"
    )
    ...
    def get_context(self, request):
        ...
        forums = Forum.objects.filter(type=Forum.FORUM_POST).select_related("parent")
...
class ForumCategoryPage(ForumBasePage):
    ...
    topics_per_page = models.IntegerField(
        default=25, help_text="Number of topics to display per page"
    )

    allow_new_topics = models.BooleanField(
        default=True, help_text="Allow users to create new topics in this forum"
    )

    show_topic_stats = models.BooleanField(
        default=True, help_text="Show topic statistics (replies, views, etc.)"
    )

    # Moderation settings
    require_approval = models.BooleanField(
        default=False, help_text="Require moderator approval for new topics"
    )
...
class ForumAnnouncementPage(ForumBasePage):
    ...
    is_pinned = models.BooleanField(
        default=True, help_text="Pin this announcement to the top of forum listings"
    )
    ...
    show_to_all = models.BooleanField(
        default=True, help_text="Show to all users, including guests"
    )

    show_to_members_only = models.BooleanField(
        default=False, help_text="Show only to registered members"
    )
...
class ForumModerationPage(ForumBasePage):
    ...
    show_pending_posts = models.BooleanField(
        default=True, help_text="Show pending posts awaiting approval"
    )

    show_reported_content = models.BooleanField(
        default=True, help_text="Show reported posts and topics"
    )

    show_user_management = models.BooleanField(
        default=True, help_text="Show user management tools"
    )

    # Auto-moderation settings
    enable_spam_detection = models.BooleanField(
        default=True, help_text="Enable automatic spam detection"
    )

    auto_approve_trusted_users = models.BooleanField(
        default=True, help_text="Automatically approve posts from trusted users"
    )
...
class ForumPageMapping(models.Model):
    ...
    wagtail_page = models.OneToOneField(
        "wagtailcore.Page", on_delete=models.CASCADE, related_name="forum_mapping"
    )

    machina_forum = models.ForeignKey(
        "forum.Forum", on_delete=models.CASCADE, related_name="wagtail_pages"
    )

    is_active = models.BooleanField(
        default=True, help_text="Is this mapping currently active?"
    )
...
class RichPost(models.Model):
    ...
    post = models.OneToOneField(
        "forum_conversation.Post", on_delete=models.CASCADE, related_name="rich_content"
    )
    ...
    ai_assisted = models.BooleanField(
        default=False, help_text="Whether AI assistance was used in creating this post"
    )

    ai_prompts_used = models.JSONField(
        null=True, blank=True, help_text="AI prompts that were used (for analytics)"
    )
...
class ForumAIUsage(models.Model):
    ...
    post = models.ForeignKey("forum_conversation.Post", on_delete=models.CASCADE)
    ...
    user_rating = models.IntegerField(
        null=True, blank=True, choices=[(i, str(i)) for i in range(1, 6)]  # 1-5 stars
    )
...
class ForumPostImage(models.Model):
    ...
    upload_order = models.PositiveIntegerField(
        default=0, help_text="Order of images in the post (0-based)"
    )
    ...
    def save(self, *args, **kwargs):
        """Auto-assign upload_order on INSERT only.

        Only new rows (no pk) get an auto-assigned order; updates preserve the
        existing order so editing metadata (e.g. alt_text) never relocates an
        image. `max_order is not None` (not truthiness) because order 0 is a valid
        max -- `0 or -1` is -1, which made the 2nd image collide on order 0 (todo 116).
        """
        if self.pk is None and (self.upload_order is None or self.upload_order == 0):
...
class PostReaction(models.Model):
    ...
    # Soft delete support
    is_active = models.BooleanField(
        default=True, help_text="Whether this reaction is currently active"
    )
    ...
    def get_post_reaction_counts(cls, post_id):
        ...
        return (
            cls.objects.filter(post_id=post_id, is_active=True)
    ...
    def get_user_reactions_for_post(cls, post_id, user_id):
        ...
        return cls.objects.filter(
            post_id=post_id, user_id=user_id, is_active=True
