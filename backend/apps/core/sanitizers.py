...
class ResponseSanitizer:
    ...
    def sanitize_response_data(
        cls, data: Any, user=None, is_debug: bool = False
    ) -> Any:
        """
    ...
    @classmethod
    def _sanitize_dict(
        cls, data: Dict[str, Any], user=None, is_debug: bool = False
    ) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            ...
            if cls._is_sensitive_field(key) and key.lower() in [
                f.lower() for f in cls.REMOVE_FIELDS
            ]:
                if is_debug:
    ...
    def _sanitize_string(cls, value: str, is_debug: bool = False) -> str:
        ...
        for pattern in suspicious_xss:
            value = re.sub(
                pattern, "[SANITIZED]", value, flags=re.IGNORECASE | re.DOTALL
            )
    ...
    def sanitize_error_response(
        cls, error_data: Any, is_debug: bool = False
    ) -> Dict[str, Any]:
        """
...
class SecureJsonResponse(JsonResponse):
    ...
    def __init__(self, data, user=None, safe=True, json_dumps_params=None, **kwargs):
        ...
        super().__init__(
            sanitized_data, safe=safe, json_dumps_params=json_dumps_params, **kwargs
        )
