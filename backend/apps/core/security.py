...
class SecurityMonitor:
    ...
    def track_failed_login_attempt(
        cls, username: str, ip_address: str
    ) -> Tuple[bool, int]:
        """
        ...
        for attempt_num in range(max_retries):
            try:
                ...
                logger.error(
                    f"{LOG_PREFIX_SECURITY} Error tracking failed attempt: {str(e)}"
                )
                if attempt_num == max_retries - 1:
                    # Last retry failed, log and return safe defaults
                    logger.error(
                        f"{LOG_PREFIX_SECURITY} All retries exhausted for {log_safe_username(username)}"
                    )
                    return False, 0
    ...
    def _send_lockout_notification(
        cls, username: str, lockout_data: Dict[str, Any]
    ) -> None:
        """
...
This is an automated security message from Plant Community.
    ...
    def track_failed_login(cls, ip_address: str, username: str = None) -> None:
        ...
        # Add new attempt
        attempts.append(
            {"timestamp": current_time, "username": username, "ip": ip_address}
        )
        ...
        if len(attempts) >= cls.MAX_FAILED_LOGINS:
            cls._trigger_security_alert(
                "brute_force_login",
                {
                    ...
                    "attempts": len(attempts),
                    "usernames": list(
                        set(a.get("username") for a in attempts if a.get("username"))
                    ),
    ...
    def track_successful_login(cls, user: User, ip_address: str) -> None:
        ...
        logger.info(
            f"{LOG_PREFIX_AUTH} Successful login: {log_safe_user_context(user)}, ip={log_safe_ip(ip_address)}"
        )
    ...
    def _check_suspicious_login(cls, user: User, ip_address: str) -> None:
        ...
        recent_logins = [
            login for login in recent_logins if current_time - login < 3600
        ]
    ...
    def track_api_request(
        cls, request: HttpRequest, endpoint: str, user: User = None
    ) -> None:
        """
    ...
    def _get_client_ip(cls, request: HttpRequest) -> str:
            ...
            if x_forwarded_for:
                ...
                for ip_str in reversed(ips):
                    try:
                        ...
                        logger.warning(
                            f"{LOG_PREFIX_SECURITY} Invalid IP in X-Forwarded-For: {ip_str}"
                        )
                        continue
        ...
        if remote_addr != UNKNOWN_IP_ADDRESS:
            try:
                ...
                logger.error(
                    f"{LOG_PREFIX_SECURITY} REMOTE_ADDR has invalid format: {log_safe_ip(remote_addr)}"
                )
...
def log_security_event(
    ...
    user_id = user.id if user else "anonymous"
    ip_address = (
        SecurityMonitor._get_client_ip(request) if request else UNKNOWN_IP_ADDRESS
    )
...
def check_rate_limit(
    user: User, action: str, limit: int, time_window: int = 60
) -> bool:
    """
