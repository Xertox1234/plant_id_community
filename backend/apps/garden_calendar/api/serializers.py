...
class EventOrganizerSerializer(serializers.ModelSerializer):
    ...
    def get_avatar_thumbnail(self, obj):
        """Return the organizer's avatar thumbnail URL, or None if no avatar.

        `avatar_thumbnail` is an imagekit ImageSpecField descriptor, not a real
        model field — ModelSerializer cannot auto-map it and would emit the raw
        CacheFile object, which crashes the JSON renderer. Mirror the working
        SerializerMethodField pattern from apps/users/serializers.py.
        """
        if obj.avatar:
...
class CommunityEventListSerializer(serializers.ModelSerializer):
    ...
    def get_user_rsvp_status(self, obj):
        ...
        if request and request.user.is_authenticated:
            # Iterate the prefetched `attendees` instead of `.get()` so the
            # lookup reuses the prefetch cache (no per-event query).
    ...
    def validate_end_datetime(self, value):
        ...
        if start_datetime and value:
            ...
            if isinstance(start_datetime, str):
                start_datetime = serializers.DateTimeField().to_internal_value(
                    start_datetime
                )
        ...
        for zone in value:
            if not isinstance(zone, str) or not zone:
                raise serializers.ValidationError(
                    "Each zone must be a non-empty string."
                )
...
class PlantListSerializer(serializers.ModelSerializer):
    ...
    growth_stage_display = serializers.CharField(
        source="get_growth_stage_display", read_only=True
    )
    ...
    def validate_garden_bed(self, value):
        ...
        if request and request.user.is_authenticated:
            if value.owner != request.user:
                raise serializers.ValidationError(
                    "You can only add plants to your own garden beds."
                )
        return value
...
class GardenBedListSerializer(serializers.ModelSerializer):
    ...
    bed_type_display = serializers.CharField(
        source="get_bed_type_display", read_only=True
    )
    sun_exposure_display = serializers.CharField(
        source="get_sun_exposure_display", read_only=True
    )
                    ...
                    raise serializers.ValidationError(
                        {
                            "non_field_errors": (
                                ...
                                f"{MAX_GARDEN_BEDS_PER_USER} active garden beds."
                            )
                        }
                    )
