...
class WeatherService:
    ...
    def get_current_weather(
        ...
        cache_key = CACHE_KEY_WEATHER_CURRENT.format(
            lat=f"{latitude:.2f}", lng=f"{longitude:.2f}"
        )
    ...
    def _parse_forecast(data: Dict[str, Any], days: int) -> Dict[str, Any]:
        ...
        for item in forecast_list:
            ...
            if date_key not in daily_forecasts:
                ...
                # Accumulate rainfall
                daily_forecasts[date_key]["rainfall"] += item.get("rain", {}).get(
                    "3h", 0
                )
    ...
    def get_watering_recommendation(
        ...
        max_temp_forecast = max(
            (day.get("temp_max", 70) for day in forecast.get("days", [])), default=70
        )
    ...
    @staticmethod
    def check_frost_risk(
        latitude: float, longitude: float, days_ahead: int = 5
    ) -> Dict[str, Any]:
        """
        ...
        min_temp = min(
            (day.get("temp_min", 100) for day in forecast.get("days", [])), default=100
        )
        if min_temp <= 40:
            return {
                "frost_risk": "moderate",
    ...
    def invalidate_cache(latitude: float, longitude: float) -> None:
        ...
        """
        cache_keys = [
            CACHE_KEY_WEATHER_CURRENT.format(
                lat=f"{latitude:.2f}", lng=f"{longitude:.2f}"
            ),
            CACHE_KEY_WEATHER_FORECAST.format(
                lat=f"{latitude:.2f}", lng=f"{longitude:.2f}"
            ),
        ]
