...
class CareScheduleService:
    ...
    def generate_initial_tasks_for_plant(plant: Plant) -> List[CareTask]:
        ...
        if watering_interval > 0:
            ...
            logger.info(
                f"[CARE_SCHEDULE] Created watering task (every {watering_interval} days)"
            )
        ...
        if fertilizing_interval > 0:
            ...
            created_tasks.append(task)
            logger.info(
                f"[CARE_SCHEDULE] Created fertilizing task (every {fertilizing_interval} days)"
            )

        # 3. Pest check task (weekly recurring)
        pest_check_interval = CareScheduleService.DEFAULT_INTERVALS["pest_check"][
            "default"
        ]
        task = CareTask.objects.create(
            plant=plant,
        ...
        if pruning_interval > 0:
            ...
            created_tasks.append(task)
            logger.info(
                f"[CARE_SCHEDULE] Created pruning task (every {pruning_interval} days)"
            )

        # 5. Harvesting reminder (if in fruiting stage)
        if growth_stage == "fruiting":
            harvesting_interval = CareScheduleService.DEFAULT_INTERVALS["harvesting"][
                "fruiting"
            ]
            task = CareTask.objects.create(
                plant=plant,
            ...
            created_tasks.append(task)
            logger.info(
                f"[CARE_SCHEDULE] Created harvesting task (every {harvesting_interval} days)"
            )

        logger.info(
            f"[CARE_SCHEDULE] Created {len(created_tasks)} initial tasks for plant {plant.uuid}"
        )
        return created_tasks

    @staticmethod
    def update_tasks_for_growth_stage_change(
        plant: Plant, old_stage: str, new_stage: str
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"[CARE_SCHEDULE] Updating tasks for plant {plant.uuid}: {old_stage} → {new_stage}"
        )
        ...
        if fertilizing_task:
            ...
            if new_interval == 0:
                ...
                updates["tasks_adjusted"].append("fertilizing")
                logger.info(
                    f"[CARE_SCHEDULE] Updated fertilizing interval to {new_interval} days"
                )

        # Update pruning interval
        pruning_task = recurring_tasks.filter(task_type="pruning").first()
        if pruning_task:
            new_interval = CareScheduleService.DEFAULT_INTERVALS["pruning"].get(
                new_stage, 0
            )
            if new_interval == 0:
                ...
                updates["tasks_adjusted"].append("pruning")
                logger.info(
                    f"[CARE_SCHEDULE] Updated pruning interval to {new_interval} days"
                )
        ...
        if new_stage == "fruiting" and old_stage != "fruiting":
            ...
            if not harvesting_task:
                harvesting_interval = CareScheduleService.DEFAULT_INTERVALS[
                    "harvesting"
                ]["fruiting"]
        ...
        logger.info(
            f"[CARE_SCHEDULE] Updated {len(updates['tasks_adjusted'])} task types"
        )
        return updates
    ...
    def generate_seasonal_tasks(user: User, season: str) -> List[CareTask]:
        ...
        """
        logger.info(
            f"[CARE_SCHEDULE] Generating seasonal tasks for user {user.id}, season: {season}"
        )
        ...
        ).filter(hardiness_zones__contains=[user_zone])

        if not templates.exists():
            logger.info(
                f"[CARE_SCHEDULE] No seasonal templates found for {season} in zone {user_zone}"
            )
            return created_tasks
    ...
    @staticmethod
    def _calculate_seasonal_task_date(
        template: SeasonalTemplate, zone: str
    ) -> Optional[datetime]:
        """
        ...
        try:
            ...
            if template.requires_no_frost:
                ...
                if zone_data and zone_data.last_frost_date:
                    ...
                    try:
                        frost_month, frost_day = map(
                            int, zone_data.last_frost_date.split("-")
                        )
                        frost_date = datetime(
                            current_year, frost_month, frost_day, 12, 0
                        )
                        ...
                        logger.warning(
                            f"[CARE_SCHEDULE] Could not parse frost date: {zone_data.last_frost_date}"
                        )
