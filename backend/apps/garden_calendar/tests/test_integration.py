...
class GardenBedPlantWorkflowTest(TestCase):
    ...
    def test_complete_garden_setup_workflow(self):
        ...
        bed_response = self.client.post(
            "/api/v1/calendar/api/garden-beds/",
            {
...
class CareTaskWorkflowTest(TestCase):
    ...
    def test_care_task_lifecycle_workflow(self):
        ...
        task_response = self.client.post(
            "/api/v1/calendar/api/care-tasks/",
            {
...
class HarvestWorkflowTest(TestCase):
    ...
    def setUp(self):
        ...
        # Create garden bed
        self.bed = GardenBed.objects.create(
            owner=self.user, name="Garden Bed", bed_type="raised"
        )
    ...
    def test_harvest_tracking_workflow(self):
        ...
        pepper_harvest = self.client.post(
            "/api/v1/calendar/api/harvests/",
            {
        ...
        summary = GardenAnalyticsService.get_harvest_summary(
            self.user, year=current_year
        )
...
class CompanionPlantingWorkflowTest(TestCase):
    ...
    def setUp(self):
        ...
        self.bed = GardenBed.objects.create(
            owner=self.user, name="Companion Garden", bed_type="raised"
        )
    ...
    def test_companion_planting_analysis_workflow(self):
        ...
        tomato_cabbage = CompanionPlantingService.check_compatibility(
            "tomato", "cabbage"
        )
        ...
        recommendations = CompanionPlantingService.get_companion_recommendations(
            "tomato"
        )
...
class MultiUserIsolationTest(TestCase):
    ...
    def setUp(self):
        ...
        self.bed1 = GardenBed.objects.create(
            owner=self.user1, name="User 1 Bed", bed_type="raised"
        )
        ...
        self.bed2 = GardenBed.objects.create(
            owner=self.user2, name="User 2 Bed", bed_type="inground"
        )
    ...
    def test_user_isolation_workflow(self):
        ...
        bed2_response = self.client.get(
            f"/api/v1/calendar/api/garden-beds/{self.bed2.uuid}/"
        )
        ...
        plant2_response = self.client.get(
...
class AnalyticsDashboardWorkflowTest(TestCase):
    ...
    def test_comprehensive_dashboard_workflow(self):
        ...
        struggling_plants = [
            p for p in needs_attention if p["health_status"] == "struggling"
        ]
