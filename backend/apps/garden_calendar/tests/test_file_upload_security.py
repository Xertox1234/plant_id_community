...
class FileUploadSecurityTestCase(TestCase):
    ...
    def test_invalid_extension_php_rejected(self):
        ...
        file = SimpleUploadedFile(
            "malicious.php", b'<?php echo "hack"; ?>', content_type="application/x-php"
        )
    ...
    def test_invalid_mime_type_rejected(self):
        ...
        file = SimpleUploadedFile(
            "fake.jpg", b'<?php echo "hack"; ?>', content_type="application/x-php"
        )
    ...
    def test_oversized_dimensions_rejected(self):
        ...
        file = SimpleUploadedFile("huge.jpg", img_io.read(), content_type="image/jpeg")
    ...
    def test_decompression_bomb_rejected(self):
        ...
        with patch("PIL.Image.open") as mock_open:
            ...
            self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
            self.assertIn(
                "error", response.data, "Response should contain 'error' field"
            )
    ...
    def test_image_count_limit_enforced(self):
        ...
        for i in range(MAX_IMAGES_PER_PLANT):
            img_io = self.create_test_image()
            file = SimpleUploadedFile(
                f"image{i}.jpg", img_io.read(), content_type="image/jpeg"
            )
        ...
        img_io = self.create_test_image()
        file = SimpleUploadedFile("extra.jpg", img_io.read(), content_type="image/jpeg")
    ...
    def test_primary_image_unsets_other_primary(self):
        ...
        img_io = self.create_test_image()
        file = SimpleUploadedFile(
            "image1.jpg", img_io.read(), content_type="image/jpeg"
        )
        ...
        img_io = self.create_test_image()
        file = SimpleUploadedFile(
            "image2.jpg", img_io.read(), content_type="image/jpeg"
        )
    ...
    def test_caption_saved_correctly(self):
        ...
        img_io = self.create_test_image()
        file = SimpleUploadedFile("test.jpg", img_io.read(), content_type="image/jpeg")
