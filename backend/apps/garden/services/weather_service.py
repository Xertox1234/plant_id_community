...
class WeatherService:
    ...
    @classmethod
    def get_forecast(
        cls, lat: float, lng: float, days: int = 5
            ...
            for item in data["list"]:
                ...
                if date_str not in daily_forecasts:
                    ...
                    daily_forecasts[date_str]["temp_min"] = min(
                        daily_forecasts[date_str]["temp_min"], item["main"]["temp_min"]
                    )
                    daily_forecasts[date_str]["temp_max"] = max(
                        daily_forecasts[date_str]["temp_max"], item["main"]["temp_max"]
                    )
    ...
    @classmethod
    def should_skip_watering(cls, lat: float, lng: float) -> bool:
        """
        ...
        if current and current.get("precipitation", 0) >= HEAVY_RAIN_INCHES:
            logger.info(
                f"[WEATHER] Skipping watering due to current rain: {current['precipitation']:.2f} inches"
            )
            return True
        ...
        if forecast and len(forecast) > 0:
            ...
            if today.get("precipitation_amount", 0) >= HEAVY_RAIN_INCHES:
                logger.info(
                    f"[WEATHER] Skipping watering due to forecast rain: {today['precipitation_amount']:.2f} inches"
                )
                return True
    ...
    def get_frost_warning(
        cls, lat: float, lng: float, days: int = 3
        ...
        return {"has_frost": False, "frost_date": None, "temp_min": None}
    ...
    def get_heatwave_warning(
        ...
        return {"has_heatwave": False, "heat_date": None, "temp_max": None}

    @classmethod
    def get_care_recommendations(cls, lat: float, lng: float) -> Dict[str, Any]:
        """
