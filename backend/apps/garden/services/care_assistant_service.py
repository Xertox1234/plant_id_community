...
class CareAssistantService:
    ...
    def generate_care_plan(
        cls, plant: GardenPlant, include_weather: bool = True
    ...
    @classmethod
    def _get_care_library_data(cls, plant: GardenPlant) -> Optional[PlantCareLibrary]:
        ...
        if care_library:
            ...
            if care_library.common_pests:
                context_parts.append(
                    f"Common pests: {', '.join(care_library.common_pests)}"
                )
    ...
    @classmethod
    def _generate_ai_care_plan(cls, context: str) -> Optional[Dict[str, Any]]:
...
Keep recommendations practical and specific to this plant and climate."""
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert gardening advisor.",
                    },
    ...
    def _fallback_care_plan(
        cls, plant: GardenPlant, care_library: PlantCareLibrary
    ) -> Dict[str, Any]:
        """Generate basic care plan from library data."""
        from ..constants import WATER_NEED_FREQUENCY

        watering_days = (
            care_library.watering_frequency_days
            or WATER_NEED_FREQUENCY.get(care_library.water_needs, 3)
        )
        ...
        return {
            ...
            "general_recommendations": care_library.care_instructions
            or "Follow standard care guidelines for this plant type",
        }

    @classmethod
    def diagnose_problem(
        cls, plant: GardenPlant, symptoms: str
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        context = cls._build_plant_context(
            plant, cls._get_care_library_data(plant), False
        )
...
Format as JSON with keys: diagnosis, severity, treatment, prevention"""
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert plant pathologist.",
                    },
                    {"role": "user", "content": prompt},
                ],
                temperature=0.7,
    ...
    @classmethod
    def get_seasonal_tasks(cls, garden: Garden, season: str) -> List[Dict[str, Any]]:
        """
        ...
        plant_list = ", ".join(
            [p.common_name for p in plants[:10]]
