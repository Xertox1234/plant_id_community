...
class Garden(models.Model):
    ...
    layout_data = models.JSONField(
        default=dict, help_text="Plant positions and canvas metadata"
    )
    ...
    climate_zone = models.CharField(
        max_length=20, blank=True, help_text="USDA hardiness zone (e.g., '7a', '9b')"
    )
    ...
    featured = models.BooleanField(
        default=False, help_text="Featured in community garden gallery"
    )
...
class CareReminder(models.Model):
    ...
    garden_plant = models.ForeignKey(
        GardenPlant, on_delete=models.CASCADE, related_name="reminders"
    )
...
class Task(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="garden_tasks"
    )
...
class PestIssue(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="pest_issues"
    )

    garden_plant = models.ForeignKey(
        GardenPlant, on_delete=models.CASCADE, related_name="pest_issues"
    )

    pest_type = models.CharField(max_length=200, help_text="Type of pest or disease")
...
class PestImage(models.Model):
    ...
    pest_issue = models.ForeignKey(
        PestIssue, on_delete=models.CASCADE, related_name="images"
    )
...
class JournalEntry(models.Model):
    ...
    garden = models.ForeignKey(
        Garden, on_delete=models.CASCADE, related_name="journal_entries"
    )
    ...
    tags = models.JSONField(
        default=list, help_text="List of tags for filtering entries"
    )
...
class JournalImage(models.Model):
    ...
    journal_entry = models.ForeignKey(
        JournalEntry, on_delete=models.CASCADE, related_name="images"
    )
...
class PlantCareLibrary(models.Model):
    ...
    water_needs = models.CharField(max_length=20, choices=WATER_NEEDS_CHOICES)

    soil_type = models.CharField(max_length=200, blank=True)

    # Hardiness zones as JSON list: ["5a", "9b"]
    hardiness_zones = models.JSONField(
        default=list, help_text="USDA hardiness zones (e.g., ['5a', '9b'])"
    )
    ...
    companion_plants = models.JSONField(
        default=list, help_text="List of compatible plant species"
    )
    enemy_plants = models.JSONField(
        default=list, help_text="List of incompatible plant species"
    )

    # Common issues
    common_pests = models.JSONField(
        default=list, help_text="List of common pests for this plant"
    )
    common_diseases = models.JSONField(
        default=list, help_text="List of common diseases"
    )
