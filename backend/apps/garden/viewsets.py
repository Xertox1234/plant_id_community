...
class GardenViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Garden.objects.filter(user=self.request.user).prefetch_related(
            Prefetch(
                "plants", queryset=GardenPlant.objects.select_related("plant_species")
            ),
            "tasks",
            "journal_entries",
        )
    ...
    @method_decorator(
        ratelimit(key="user", rate=RATE_LIMIT_GARDEN_CREATE, method="POST")
    )
    ...
    def featured(self, request):
        ...
        """
        gardens = (
            Garden.objects.filter(visibility="public", featured=True)
            .select_related("user")
            .prefetch_related(
                Prefetch(
                    "plants",
    ...
    def public(self, request):
        ...
        """
        gardens = (
            Garden.objects.filter(visibility="public")
            .select_related("user")
            .prefetch_related(
                Prefetch(
                    "plants",
...
class GardenPlantViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            GardenPlant.objects.filter(garden__user=self.request.user)
            .select_related("garden", "plant_species")
            .prefetch_related(
                "reminders", "pest_issues__images", "journal_entries__images"
            )
        )
...
class CareReminderViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return CareReminder.objects.filter(user=self.request.user).select_related(
            "user", "garden_plant__garden"
        )
    ...
    def upcoming(self, request):
        ...
        reminders = (
            self.get_queryset()
            .filter(completed=False, scheduled_date__lte=end_date)
...
class PestIssueViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            PestIssue.objects.filter(user=self.request.user)
...
class JournalEntryViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            ...
            .select_related("user", "garden", "garden_plant")
            .prefetch_related("images")
        )
