...
class BlogPostPageViewSetRoutingParityTestCase(TestCase):
    """Drift guard: every @action on BlogPostPageViewSet must resolve to a
    real, correctly-bound path() entry in urls.py, or it's unreachable dead
    code (todo 307). Routes are generated by
    `extra_action_urlpatterns()` (urls.py) rather than hand-written, which
    makes "forgot to route a new action" structurally impossible — this
    test is a smoke check on that generator's output, not a substitute for
    it."""
    ...
    def test_all_extra_actions_are_routed(self):
        ...
        for action_func in BlogPostPageViewSet.get_extra_actions():
            ...
            self.assertIn(
                expected,
                patterns_by_str,
                f"Action '{action_func.__name__}' has no path() entry in "
                f"urls.py (expected pattern: {expected!r}) — Wagtail's "
                "API router does not auto-mount @action-decorated methods.",
            )
            matched = patterns_by_str[expected]
            ...
            expected_name = f"blog-posts-{action_func.url_name}"
            self.assertEqual(
                matched.name,
                expected_name,
                f"Path {expected!r} is named {matched.name!r}, expected "
                f"{expected_name!r} — a typo'd name breaks any "
                "reverse()/{% url %} call even though the route itself "
                "still works.",
            )
