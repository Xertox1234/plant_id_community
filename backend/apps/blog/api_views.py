...
class PlantLookupView(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            if not query:
                return JsonResponse(
                    {"success": False, "error": "Plant name/query is required"},
                    status=400,
                )
            ...
            if block_type == "plant_spotlight":
                result = self.auto_population_service.populate_plant_spotlight_fields(
                    query, user
                )
            else:  # care_instructions
                result = self.auto_population_service.populate_care_instructions_fields(
                    query, user
                )
    ...
    def get(self, request):
        ...
        try:
            ...
            if cached_suggestions:
                return JsonResponse(
                    {"success": True, "suggestions": cached_suggestions, "cached": True}
                )
            ...
            for name in scientific_matches:
                suggestions.append({"value": name, "label": name, "type": "scientific"})
            ...
            return JsonResponse(
                {"success": True, "suggestions": suggestions[:limit], "cached": False}
            )
...
def generate_ai_content(request):
    ...
    try:
        ...
        if content_type == "description":
            prompt = PlantAIPrompts.get_plant_description_prompt(
                plant_name, existing_data
            )

        elif content_type == "care_instructions":
            # This is a general care instructions prompt
            prompt = PlantAIPrompts.get_care_instructions_prompt(
                plant_name, "general", existing_data
            )

        elif content_type == "special_notes":
            prompt = PlantAIPrompts.get_care_instructions_prompt(
                plant_name, "special_notes", existing_data
            )
            ...
            prompt = PlantAIPrompts.get_care_instructions_prompt(
                plant_name, content_type, existing_data
            )
