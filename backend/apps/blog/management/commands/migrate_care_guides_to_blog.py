...
class Command(BaseCommand):
    help = (
        "Migrate PlantCareGuide data to BlogPostPage entries under Plant Care category"
    )
    ...
    def handle(self, *args, **options):
        ...
        if guide_count == 0:
            self.stdout.write(
                self.style.SUCCESS("No plant care guides found to migrate.")
            )
            return
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(
                    "No BlogIndexPage found. Please create a blog index page first."
                )
            )
            return
            ...
            for care_guide in care_guides:
                try:
                    ...
                    if blog_post:
                        ...
                        self.stdout.write(
                            self.style.SUCCESS(
                                f"✓ Migrated: {care_guide.plant_species.scientific_name}"
                            )
                        )
                    else:
                        self.stdout.write(
                            self.style.WARNING(
                                f"⚠ Skipped: {care_guide.plant_species.scientific_name}"
                            )
                        )
                except Exception as e:
                    self.stdout.write(
                        self.style.ERROR(
                            f"✗ Failed to migrate {care_guide.plant_species.scientific_name}: {e}"
                        )
                    )
                    if not dry_run:
                        raise CommandError(
                            f"Migration failed for {care_guide.plant_species.scientific_name}: {e}"
                        )

            if dry_run:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"DRY RUN: Would migrate {migrated_count} care guides to blog posts"
                    )
                )
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Successfully migrated {migrated_count} care guides to blog posts"
                    )
                )
    ...
    def create_plant_care_category(self, dry_run):
        ...
        if dry_run:
            try:
                ...
                self.stdout.write(
                    f"Plant Care category already exists: {category.name}"
                )
                return category
    ...
    def migrate_care_guide_to_blog_post(
        self, care_guide, author, blog_index, category, dry_run
    ):
        ...
        if existing_post:
            self.stdout.write(
                f"Blog post already exists for {plant_species.scientific_name}"
            )
        ...
        introduction = (
            care_guide.quick_care_summary
            or f"Complete care guide for {plant_species.display_name}"
        )
        ...
        blog_post = BlogPostPage(
            ...
            difficulty_level=(
                ...
                if care_guide.care_difficulty != "very_easy"
                else "beginner"
            ),
        ...
        if any(
            ...
            care_block = (
                "care_instructions",
                {
                    ...
                    "watering": care_guide.watering_description
                    or "Watering information not specified.",
                    "lighting": care_guide.light_description
                    or "Light requirements not specified.",
                    ...
                    "special_notes": care_guide.seasonal_notes
                    or care_guide.common_problems
                    or "",
                },
            )
            ...
            try:
                for block in care_guide.care_content:
                    ...
                    if block.block_type == "heading":
                        ...
                        blocks.append(
                            ("heading", f"💡 {tip_data.get('tip_title', 'Care Tip')}")
                        )
                        ...
                        blocks.append(
                            ("paragraph", seasonal_data.get("care_instructions", ""))
                        )
                        if seasonal_data.get("special_notes"):
                            blocks.append(
                                (
                                    "paragraph",
                                    f"Special notes: {seasonal_data['special_notes']}",
                                )
                            )
                        ...
                        blocks.append(
                            ("heading", f"⚠️ Problem: {problem_data.get('problem', '')}")
                        )
                        if problem_data.get("symptoms"):
                            blocks.append(
                                ("paragraph", f"Symptoms: {problem_data['symptoms']}")
                            )
                        if problem_data.get("solution"):
                            blocks.append(
                                ("paragraph", f"Solution: {problem_data['solution']}")
                            )
                        if problem_data.get("prevention"):
                            blocks.append(
                                (
                                    "paragraph",
                                    f"Prevention: {problem_data['prevention']}",
                                )
                            )
