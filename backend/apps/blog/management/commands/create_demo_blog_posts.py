...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            with transaction.atomic():
                ...
                demo_posts = self.create_demo_posts(
                    author, blog_index, categories, dry_run, overwrite
                )

                if dry_run:
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"DRY RUN: Would create {len(demo_posts)} demo blog posts"
                        )
                    )
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"Successfully created {len(demo_posts)} demo blog posts"
                        )
                    )
                    self.stdout.write("")
    ...
    def setup_author(self, author_email, dry_run):
        ...
        try:
            ...
            self.stdout.write(
                f"Using existing author: {author.username} ({author.email})"
            )
            return author
        except User.DoesNotExist:
            if dry_run:
                self.stdout.write(
                    f"Would create demo author with email: {author_email}"
                )
                return None
    ...
    def setup_blog_index(self, dry_run):
        ...
        try:
            ...
            self.stdout.write(
                self.style.WARNING(
                    f"Multiple blog indexes found, using: {blog_index.title}"
                )
            )
            return blog_index
    ...
    def create_demo_posts(self, author, blog_index, categories, dry_run, overwrite):
        ...
        for post_data in demo_posts_data:
            try:
                ...
                existing_post = BlogPostPage.objects.filter(
                    slug=post_data["slug"]
                ).first()
                if existing_post and not overwrite:
                    ...
                    self.stdout.write(
                        f'Overwriting existing post: {post_data["title"]}'
                    )
    ...
    def get_low_light_content_blocks(self):
        ...
        return [
            ...
            (
                "paragraph",
                "<p>Living in a space with limited natural light doesn't mean you have to give up your plant dreams! These incredible low-light champions have evolved to thrive in the understory of forests, making them perfectly adapted to dimmer indoor conditions.</p>",
            ),
    ...
    def get_propagation_content_blocks(self):
        ...
        return [
            ...
            (
                "paragraph",
                "<p>As daylight hours increase and temperatures warm up, plants naturally enter their most active growth phase. This makes spring the ideal time to take cuttings, as plants have the energy to develop new roots and recover quickly from pruning.</p>",
            ),
            ...
            (
                "paragraph",
                "<p><strong>Step 1:</strong> Cut a 4-6 inch stem with at least one node (the bump where leaves grow from)<br><strong>Step 2:</strong> Remove lower leaves that would sit underwater<br><strong>Step 3:</strong> Place cutting in clean water, change water every 3-5 days<br><strong>Step 4:</strong> Wait for roots to develop (1-4 weeks depending on plant)<br><strong>Step 5:</strong> Plant in soil when roots are 1-2 inches long</p>",
            ),
            ("heading", "Soil Propagation: For Faster Establishment"),
            (
                "paragraph",
                "<p>Some plants prefer to root directly in soil. Use a well-draining potting mix, keep consistently moist (not soggy), and provide bright, indirect light. Cover with a clear plastic bag to maintain humidity if needed.</p>",
            ),
            (
                "care_instructions",
                {
