...
class BlogStreamBlocks(blocks.StreamBlock):
    ...
    quote = blocks.StructBlock(
        [
            ...
            (
                "attribution",
                blocks.CharBlock(required=False, help_text="Who said this quote?"),
            ),
        ],
        icon="openquote",
...
class BlogPostView(models.Model):
    ...
    user_agent = models.CharField(
        max_length=255, blank=True, help_text="Browser user agent string"
    )

    referrer = models.URLField(
        blank=True, help_text="Referrer URL (where the user came from)"
    )

    viewed_at = models.DateTimeField(
        auto_now_add=True, help_text="When the post was viewed"
    )
...
class BlogSeries(models.Model):
    ...
    is_completed = models.BooleanField(
        default=False, help_text="Mark series as completed"
    )
...
class BlogIndexPage(BlogBasePage):
    ...
    # Page content
    introduction = RichTextField(blank=True, help_text="Introduction text for the blog")

    # Display settings
    posts_per_page = models.IntegerField(
        default=12, help_text="Number of posts to display per page"
    )

    show_featured_posts = models.BooleanField(
        default=True, help_text="Show featured posts section"
    )

    show_categories = models.BooleanField(
        default=True, help_text="Show categories section"
    )
    ...
    def get_context(self, request):
        ...
        # Get blog posts
        blog_posts = (
            BlogPostPage.objects.live().public().order_by("-first_published_at")
        )
...
class BlogAuthorPage(BlogBasePage):
    ...
    author = models.OneToOneField(
        User, on_delete=models.PROTECT, help_text="The user this author page represents"
    )

    bio = RichTextField(help_text="Author bio and background information")
...
class BlogAuthorExpertise(TaggedItemBase):
    content_object = ParentalKey(
        "blog.BlogAuthorPage", related_name="expertise_items", on_delete=models.CASCADE
    )
...
class BlogPostPage(HeadlessPreviewMixin, BlogBasePage):
    ...
    author = models.ForeignKey(
        User, null=False, on_delete=models.PROTECT, help_text="Post author"
    )

    publish_date = models.DateField(help_text="Date to publish this post")

    # Content
    introduction = RichTextField(blank=False, help_text="Brief introduction or excerpt")
    ...
    categories = ParentalManyToManyField(
        BlogCategory, blank=True, help_text="Categories for this post"
    )

    tags = ClusterTaggableManager(
        through=BlogPostTag, blank=True, help_text="Tags for this post"
    )
    ...
    series_order = models.PositiveIntegerField(
        null=True, blank=True, help_text="Order within the series"
    )
    ...
    is_featured = models.BooleanField(
        default=False, help_text="Feature this post on the blog index"
    )
    ...
    content_panels = [
        ...
        # Content
        AIFieldPanel(
            "introduction", prompts=["page_description_prompt"]
...
class BlogComment(models.Model):
    ...
    author = models.ForeignKey(
        User, on_delete=models.CASCADE, help_text="Comment author"
    )
    ...
    is_approved = models.BooleanField(
        default=True, help_text="Is this comment approved for display?"
    )

    is_flagged = models.BooleanField(
        default=False, help_text="Has this comment been flagged by users?"
    )

    flag_count = models.PositiveIntegerField(
        default=0, help_text="Number of times this comment has been flagged"
    )
...
class BlogNewsletter(models.Model):
    ...
    first_name = models.CharField(
        max_length=100, blank=True, help_text="Subscriber first name"
    )
    ...
    categories = models.ManyToManyField(
        BlogCategory, blank=True, help_text="Categories of interest"
    )

    # Plant-specific preferences
    plant_types_interest = models.TextField(
        blank=True, help_text="Plant types of interest (comma-separated)"
    )
    ...
    source = models.CharField(
        max_length=100, blank=True, help_text="How did they find us?"
    )

    ip_address = models.GenericIPAddressField(
        null=True, blank=True, help_text="IP address when subscribed"
    )
