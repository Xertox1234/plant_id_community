...
class PlantAIPrompts:
    ...
    @staticmethod
    def get_plant_description_prompt(
        plant_name: str, existing_data: Optional[Dict] = None
    ) -> str:
        """
...
- Focus on what makes this plant special"""
        ...
        if existing_data:
            ...
            if existing_data.get("growth_habit"):
                additional_context.append(
                    f"Growth habit: {existing_data['growth_habit']}"
                )
            if existing_data.get("native_regions"):
                additional_context.append(
                    f"Native to: {existing_data['native_regions']}"
                )
    ...
    @staticmethod
    def get_care_instructions_prompt(
        plant_name: str, care_type: str, existing_data: Optional[Dict] = None
    ) -> str:
        """
...
Write encouraging, practical advice that helps ensure success.""",
        ...
        base_prompt = care_prompts.get(
            care_type, f"Write {care_type} care instructions for {plant_name}."
        )
        ...
        if existing_data:
            ...
            if care_type == "watering" and existing_data.get("water_requirements"):
                context_info.append(
                    f"Water needs: {existing_data['water_requirements']}"
                )
            elif care_type == "lighting" and existing_data.get("light_requirements"):
                context_info.append(
                    f"Light needs: {existing_data['light_requirements']}"
                )
            elif care_type == "temperature" and existing_data.get("hardiness_zones"):
                context_info.append(
                    f"Hardy in zones: {existing_data['hardiness_zones']}"
                )
...
def get_ai_prompt_for_block(block_type: str, field_name: str, context: Dict) -> str:
    ...
    if block_type == "plant_spotlight":
        if field_name == "description":
            return PlantAIPrompts.get_plant_description_prompt(
                plant_name, existing_data
            )
        ...
        if field_name in [
            ...
            return PlantAIPrompts.get_care_instructions_prompt(
                plant_name, field_name, existing_data
            )
