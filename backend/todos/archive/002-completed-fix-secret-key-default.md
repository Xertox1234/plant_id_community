...
`status: pending` -> `completed`. `backend/plant_community_backend/settings.py`
defines `INSECURE_PATTERNS` (:67, including `django-insecure`), rejects any
SECRET_KEY matching one (:99), and enforces `len(SECRET_KEY) >= 50` (:147).
