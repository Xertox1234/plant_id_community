...
def test_email_templates():
    ...
    for template_test in templates_to_test:
        try:
            ...
            html_content = render_to_string(
                f"emails/{template_test['template']}.html", template_test["context"]
            )

            # Test text template
            text_content = render_to_string(
                f"emails/{template_test['template']}.txt", template_test["context"]
            )
            ...
            print(
                f"  ✅ {template_test['name']} - HTML: {len(html_content)} chars, Text: {len(text_content)} chars"
            )
            success_count += 1
    ...
    print("\n" + "=" * 50)
    print(
        f"📊 Results: {success_count}/{len(templates_to_test) + len(auth_templates)} templates passed"
    )
