...
def test_user_audit():
    ...
    user, created = User.objects.get_or_create(
        username="test_audit_user", defaults={"email": "test_audit@example.com"}
    )
    ...
    logs = LogEntry.objects.filter(
        content_type__model="user", object_pk=str(user.pk)
    ).order_by("-timestamp")
...
def test_plant_species_audit():
    ...
    # Check audit log
    logs = LogEntry.objects.filter(
        content_type__model="plantspecies", object_pk=str(species.pk)
    ).order_by("-timestamp")

    if logs.exists():
        latest_log = logs.first()
        print(
            f"✓ Found {logs.count()} audit log entries for species {species.scientific_name}"
        )
        print(f"  Latest action: {latest_log.get_action_display()}")
        print(f"  Timestamp: {latest_log.timestamp}")
        if latest_log.changes:
...
def test_audit_log_queries():
    ...
    user_logs = LogEntry.objects.filter(content_type__model="user").count()
    ...
    if test_users.exists():
        ...
        user_actions = LogEntry.objects.filter(actor=test_user).count()
    ...
    recent_logs = LogEntry.objects.filter(timestamp__gte=recent_cutoff).count()
