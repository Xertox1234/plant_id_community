...
SPECTACULAR_SETTINGS = {
    ...
    "TAGS": [
        ...
        },
        {
            ...
            "description": "Plant identification using AI (Plant.id + PlantNet)",
        },
...
SIMPLE_JWT = {
    ...
    "ACCESS_TOKEN_LIFETIME_DEBUG": (
        timedelta(
            minutes=config("JWT_ACCESS_TOKEN_LIFETIME_DEBUG", default=120, cast=int)
        )
        if DEBUG
        else None
    ),
    ...
    try:
        ...
        if "handlers" in LOGGING.get("root", {}):
            LOGGING["root"]["handlers"] = [
                h for h in LOGGING["root"]["handlers"] if h != "file"
            ]
...
LOGGING = {
    ...
    "handlers": {
        "console": {
            ...
            + (["request_id"] if _HAS_REQUEST_ID else []),
...
CELERY_BROKER_URL = config(
    "CELERY_BROKER_URL", default=config("REDIS_URL", default="redis://127.0.0.1:6379/1")
)
...
PERMISSIONS_POLICY = {
    ...
    "geolocation": [
        "self"
    ],  # Geolocation only from same origin (plant location tracking)
...
SECURE_REFERRER_POLICY = (
    "strict-origin-when-cross-origin"  # Privacy-preserving referrer policy
)
    ...
    CONTENT_SECURITY_POLICY = {
        "DIRECTIVES": {
            ...
            "img-src": (
                ...
                "https:",
                "blob:",
            ),  # Allow HTTPS images (plant photos from external sources)
            "media-src": ("'self'",),
            "object-src": ("'none'",),  # Block Flash, Java applets, etc.
...
ENABLE_AI_CONTENT_GENERATION = config(
    "ENABLE_AI_CONTENT_GENERATION", default=True, cast=bool
)
...
def validate_environment():
    ...
    if not redis_url:
        if not DEBUG:
            ...
            warnings.append(
                "Redis not configured - caching will use in-memory fallback (not recommended)"
            )
    else:
    ...
    optional_api_checks = [
        (
            "TREFLE_API_KEY",
            TREFLE_API_KEY,
            ENABLE_TREFLE_ENRICHMENT,
            "Plant data enrichment",
        ),
        (
            "PLANT_HEALTH_API_KEY",
            PLANT_HEALTH_API_KEY,
            ENABLE_DISEASE_DIAGNOSIS,
            "Disease diagnosis",
        ),
        (
            "OPENAI_API_KEY",
    ...
    google_configured = bool(
        config("GOOGLE_OAUTH2_CLIENT_ID", default="")
        and config("GOOGLE_OAUTH2_CLIENT_SECRET", default="")
    )
    github_configured = bool(
        config("GITHUB_CLIENT_ID", default="")
        and config("GITHUB_CLIENT_SECRET", default="")
    )

    if not google_configured and not github_configured:
        warnings.append(
            "No OAuth providers configured - users can only register with username/password"
        )
    ...
    if EMAIL_BACKEND == "django.core.mail.backends.console.EmailBackend" and not DEBUG:
        warnings.append(
            "Email backend is console in production - emails will not be sent"
        )
    ...
    security_checks = [
        (
            not SECURE_SSL_REDIRECT and not DEBUG,
            "SECURE_SSL_REDIRECT should be True in production",
        ),
        (
            not SESSION_COOKIE_SECURE and not DEBUG,
            "SESSION_COOKIE_SECURE should be True in production",
        ),
        (
            not CSRF_COOKIE_SECURE and not DEBUG,
            "CSRF_COOKIE_SECURE should be True in production",
        ),
        (
            not CSRF_COOKIE_HTTPONLY,
            "CSRF_COOKIE_HTTPONLY should be True (prevents XSS token theft)",
        ),
    ]
    ...
    if not warnings and not critical_errors:
        logger.info(
            "[ENV VALIDATION] ✅ All critical environment variables properly configured"
        )
        if DEBUG:
