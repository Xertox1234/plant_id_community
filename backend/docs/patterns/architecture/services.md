...
### Performance Impact (Circuit Breaker)
...
### Performance Impact (Distributed Lock)
...
### Pattern: Gate `on_commit` Registration on the Preceding Write's Success

**Problem**: A `try/except` added around a DB write (e.g. to scope a failure to
a savepoint instead of poisoning the ambient transaction) does not, by itself,
stop a *following* `transaction.on_commit(...)` call from still registering —
`except` only stops the exception from propagating; execution falls through to
whatever comes after the `try/except` block regardless of which branch ran. If
an `on_commit`-deferred side-effect (a push notification, a webhook, a cache
warm) sits after the block instead of inside it, a caught write failure still
delivers that side-effect — e.g. a push saying "X replied" with no
`Notification` row for the recipient to actually see.

**Incorrect** ❌ (todo 253 slice 1, `forum_host/notifications.py`, caught by
kimi-review's commit gate — the 4 domain reviewers that reviewed this file
missed it, since their pass ran before this try/except existed):

```python
try:
    with transaction.atomic():
        create_notifications(recipients=[topic_author], ...)
except Exception:
    logger.exception("failed to persist notification")

# Runs unconditionally — even when the except above just fired.
transaction.on_commit(lambda: send_forum_push.delay(...))
```
...
try:
    with transaction.atomic():
        create_notifications(recipients=[topic_author], ...)
    # Only reached if the write above didn't raise.
    transaction.on_commit(lambda: send_forum_push.delay(...))
except Exception:
    logger.exception("failed to persist notification")
```

**Rule**: when a guarded write and an `on_commit` side-effect are adjacent,
the registration call belongs as the *last statement inside* the `try`, not
after the `try/except`. Test it directly — mock the write to raise and assert
